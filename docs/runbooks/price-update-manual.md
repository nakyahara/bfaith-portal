...
- Amazon・LINEギフトは対象外。チェックリストとしてだけ記録される
  (Amazon = 別ツールがあるため / LINEギフト = 価格だけを変える API が無いため。**LINEギフトの価格は LINEギフトの管理画面で直す**)
- 🚨**チェックを入れた行だけが記録される**。売価を入れてもチェックが無いと記録されず、モールへも送られない
...
- いちばん左上のチェックボックスは **「単品の行をすべて選ぶ」**。**セットの行と ⛔ の行は選ばれない**
  (売価がまだ空の行は選ばれる / 外すときは全部外れる)

![詳しく表示](images/price-update/02-detail.png)

左下に **「記録すると N 行 — 送れる X / ⛔ Y / 売価が未入力 Z / 手動更新 W (Amazon・LINEギフト)」** が出る。
記録API と同じ数え方なので、この N がそのまま履歴の行数になる
(**手動更新モールの行は選ばなくても足される**)。ここを見てから「この内容を記録する」。

🚨**「送れる 0」のまま記録しない**。手動更新モールの行はチェック無しでも足されるので、
チェックを1つも入れていなくても記録だけはできてしまう (= 履歴はできるのに、モールへは何も送られない)。
**選んだ行が全部 ⛔ / 売価が未入力のときも同じ**で、その履歴からは1件も送れない。
どちらの場合も「⚠ 送れる行がありません」と赤字が出て、記録を押すと確認を聞かれる
(チェックリストとしてだけ残したいときは、確認して進めば記録できる)。
**2026-09-07 に実際にこれが起きた** (`lightbluetb-100` の履歴が LINEギフト1行だけででき、
現場は価格を変えたつもりになっていた)。
...
赤い枠が **「送る行がありません — …」**になっていたら、そこに理由が書いてある。よくあるのは
「行のチェックを入れ忘れた」「⛔ で止まっている」。
⚠️**「結果が不明」の行がある場合は、送られたかどうか分からない**ので、
自動では送り直さず**モールの画面で実際の価格を確かめる** (この文言も枠の中に出る)。
...
| 手動更新まち | Amazon・LINEギフトの行 | **モールの管理画面で手で直してから**「手動更新した」を押す (このボタンは印を付けるだけで、価格は変わらない) |
