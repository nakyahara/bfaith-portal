...
- 間違えた紐付けは 各アプリの管理画面「スタッフマスタ (紐付け直し)」で直す (`relinkRosterWorker`)。**同じ人を指す全アプリの鏡をまとめて付け替える** (`registerMirror`)。全部付け替えられたら元の行の役割 iroha を外し、自動採番の行で他に役割が無ければ無効に。PIN は引き継ぐ (先が利用者なら引き継がない)
