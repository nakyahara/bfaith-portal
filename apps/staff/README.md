...
- 間違えた紐付けは 各アプリの管理画面「スタッフマスタ (紐付け直し)」で直す (`relinkRosterWorker`)。**記録 `staff_merges` (from → to) を staff.db に残し、どの鏡も写すときに冪等に適用する** (プロセス内の `registerMirror` はその場で揃えるための最善努力)。全部付け替えられたら元の行の役割 iroha を外し、自動採番の行で他に役割が無ければ無効に。PIN は引き継ぐ (先が利用者なら引き継がない)。どこかの鏡で to が既に別の行なら記録せず、その鏡は 2 行のまま
