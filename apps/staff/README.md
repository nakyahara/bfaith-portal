...
- 画面: `/apps/staff/` (**管理者のみ**)。追加・行ごと編集 (楽観ロック)・無効化/再有効化。削除はしない (他アプリの履歴が `staff.id` を参照)
  ...
  - **既定では 8 列 + 操作**だけ出す (名前 / 短い表記 / 管理番号 / 区分 / 状態 / 役割 / PIN / 並び + 操作)。
    よみ / ポータルmail / 入社日 / 退職日 / メモ は「詳細列」のチェックで出る (localStorage に記憶)。
...
| `iroha` | いろは現場 | いろは在庫化・FBA箱詰め (iPad) の名前タップに**出る**。**この役割だけの人と 区分 `iroha` (利用者) は `/export` (miniPC 向け) に出さない** — 取込側は対象外の人でも未紐付けの作業者と名前が一致すれば紐付けて無効にするため、いろはだけの人を渡すと同名の別人がピッキングから消える。⚠ 以前 export に居た社員の役割を いろは だけにすると、取込側に「スタッフマスタから消えています」の警告が出て有効のまま残る → ピッキングの管理画面で無効にする |
...
- 間違えた紐付けは 各アプリの管理画面「スタッフマスタ (紐付け直し)」で直す (`relinkRosterWorker`)。**記録 `staff_merges` (from → to) を staff.db に残し、どの鏡も写すときに冪等に適用する** (プロセス内の `registerMirror` はその場で揃えるための最善努力)。全部付け替えられたら元の行の役割 iroha を外し、自動採番の行で他に役割が無ければ無効に。PIN は引き継ぐ (先が利用者なら引き継がない)。どこかの鏡で to が既に別の行なら記録せず、その鏡は 2 行のまま。適用は from→**終端** (連鎖 A→B→C は A→C) に解いてから、変わらなくなるまで繰り返す (未同期の鏡でも 1 回で収束)。統合済みの行を先に選ぶ・元の行へ戻す (循環) は断る
- 移行 1 行ぶん・追加 は staff.db の 1 トランザクション (BEGIN IMMEDIATE)。両アプリが同時に起動しても同じ人が 2 行にならない
- `staff_meta.pin_ever_set` = 職員PIN を一度でも設定した (どの経路でも)。FBA箱詰めの名簿 bootstrap (PIN 持ちが 0 人なら iPad から無ゲート登録) はこれも見て恒久に閉じる
