#!/usr/bin/env node
/**
 * sync-yahoo-data-daily.js — Yahoo!ストクリ統計CSV mirror sync (mall-csv-fetcher P1-Y)
 *
 * fact_yahoo_* 6種を Render mirror へ sync。sync-rakuten-data-daily.js と同型
 * (chunk POST + ledger + scope_clear_per_run)。過去分は再DLで変動しうるため
 * scope 内は 0 行日付も含めて clear → 再投入。
 *
 * 使い方:
 *   DATA_DIR=... node apps/warehouse/sync-yahoo-data-daily.js --days 110 --dry-run
 *   DATA_DIR=... node apps/warehouse/sync-yahoo-data-daily.js --from 2026-04-01 --to 2026-07-10
 *   DATA_DIR=... node apps/warehouse/sync-yahoo-data-daily.js --days 110 --entity yahoo_keyword_daily
 *
...
// ============================================================
// entity 定義 (全て SELECT * + dropCols。mirror 側は英語列)
// ============================================================
const YAHOO_DROP_COLS = ['source_file', 'import_id', 'updated_at'];
const ENTITIES = [
  {
    name: 'yahoo_store_device_daily',
    clearMetaKey: 'clear_yahoo_store_device_dates',
    selectSql: `SELECT * FROM fact_yahoo_store_device_daily WHERE date_jst >= ? AND date_jst <= ? ORDER BY date_jst, device`,
    hashRow: (r) => ({ d: r.date_jst, dev: r.device, s: r.sales_yen, pv: r.pageviews }),
    sampleLog: (r) => `date=${r.date_jst} dev=${r.device} sales=¥${r.sales_yen}`,
  },
  {
    name: 'yahoo_inflow_daily',
    clearMetaKey: 'clear_yahoo_inflow_dates',
    selectSql: `SELECT * FROM fact_yahoo_inflow_daily WHERE date_jst >= ? AND date_jst <= ? ORDER BY date_jst`,
    hashRow: (r) => ({ d: r.date_jst, i: r.inflow_visitors, b: r.purchase_visitors }),
    sampleLog: (r) => `date=${r.date_jst} inflow=${r.inflow_visitors}`,
  },
  {
    name: 'yahoo_user_attr_daily',
    clearMetaKey: 'clear_yahoo_user_attr_dates',
    selectSql: `SELECT * FROM fact_yahoo_user_attr_daily WHERE date_jst >= ? AND date_jst <= ? ORDER BY date_jst, gender, age_band, buyer_class`,
    hashRow: (r) => ({ d: r.date_jst, g: r.gender, a: r.age_band, c: r.buyer_class, v: r.visitors }),
    sampleLog: (r) => `date=${r.date_jst} ${r.gender}/${r.age_band}/${r.buyer_class}=${r.visitors}`,
  },
  {
    name: 'yahoo_flash_hourly',
    clearMetaKey: 'clear_yahoo_flash_dates',
    // 速報は当日行を持つ (取込→syncの順で走るため当日clearしても欠落しない)
    extendToToday: true,
    selectSql: `SELECT * FROM fact_yahoo_flash_hourly WHERE date_jst >= ? AND date_jst <= ? ORDER BY date_jst, hour_slot, device`,
    hashRow: (r) => ({ d: r.date_jst, h: r.hour_slot, dev: r.device, s: r.sales_yen }),
    sampleLog: (r) => `date=${r.date_jst} ${r.hour_slot} dev=${r.device} sales=¥${r.sales_yen}`,
  },
  {
    name: 'yahoo_item_daily',
    clearMetaKey: 'clear_yahoo_item_dates',
    selectSql: `SELECT * FROM fact_yahoo_item_daily WHERE date_jst >= ? AND date_jst <= ? ORDER BY date_jst, item_code, sub_code`,
    hashRow: (r) => ({ d: r.date_jst, i: r.item_code, sc: r.sub_code, s: r.sales_yen, v: r.visitors }),
    sampleLog: (r) => `date=${r.date_jst} item=${r.item_code} sales=¥${r.sales_yen}`,
  },
  {
    name: 'yahoo_keyword_daily',
    clearMetaKey: 'clear_yahoo_keyword_dates',
    selectSql: `SELECT * FROM fact_yahoo_keyword_daily WHERE date_jst >= ? AND date_jst <= ? ORDER BY date_jst, keyword`,
    hashRow: (r) => ({ d: r.date_jst, k: r.keyword, f: r.inflow, s: r.sales_yen }),
    sampleLog: (r) => `date=${r.date_jst} kw=${r.keyword.slice(0, 16)} inflow=${r.inflow}`,
  },
].map((e) => ({
  contractVersion: 1,
  scopeDates: (range) => enumerateScopeDates(range.from, range.to),
  selectParams: (range) => [range.from, range.to],
  dropCols: YAHOO_DROP_COLS,
  ...e,
}));
...
async function syncEntity(entity) {
  ...
  try {
    ...
    if (!exists) {
      console.error(`  ✗ source table ${tableName} が未作成。先に import-yahoo-data.js を実行してください`);
