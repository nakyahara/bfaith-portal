#!/usr/bin/env node
/**
 * sync-aupay-finance-daily.js — au PAY マーケット Phase 1 A-2 entity-driven sync
 *
 * f_aupay_finance_sku_daily_v1 → mirror_aupay_finance_sku_daily に sync。
 * Yahoo Phase 1 Y-3a (sync-yahoo-finance-daily.js) / 楽天 #R-3a と同型。
 *
 * 使い方:
 *   DATA_DIR=... node apps/warehouse/sync-aupay-finance-daily.js --month 2026-05 --dry-run
 *   DATA_DIR=... node apps/warehouse/sync-aupay-finance-daily.js --month 2026-05
 *
 * env:
 *   DATA_DIR             必須
...
if (!contract) {
  ...
  console.error(`  Run: node -e "..." (sql/sync/seed_aupay_finance_sku_daily.sql 相当を適用)`);
  process.exit(1);
}
...
const rows = db.prepare(`
  SELECT * FROM f_aupay_finance_sku_daily_v1
  WHERE date_jst >= ? AND date_jst <= ?
  ORDER BY date_jst, aupay_sku_key
...
import('node:os').then(async (os) => {
  ...
  for (let i = 0; i < chunks.length; i++) {
    ...
    const body = {
      ...
      meta: isFirst ? { clear_aupay_finance_dates: distinctDates } : {},
      payload,
    };
