...
// 削除時は m_products.売上分類 を「手動登録なし」で解決し直す。
//   単品/例外 → NULL に戻して未登録一覧に再出現させる
//   セット     → 構成品から導出し直す (構成品も未登録なら NULL で一覧に出る)
// さらに、この商品を構成品に持つセットの導出値も更新する。
...
router.delete('/api/sales_class/:sku', (req, res) => {
  ...
  let deleted = 0, setsUpdated = 0;
  const tx = db.transaction(() => {
    const old = db.prepare('SELECT * FROM product_sales_class WHERE sku = ?').get(sku);
    deleted = db.prepare('DELETE FROM product_sales_class WHERE sku = ?').run(sku).changes;
    if (old) auditLog(db, 'product_sales_class', sku, 'DELETE', old, null);

    // 手動登録を外した状態で再解決する (単品とセットで同じ解決順を使う)
    const kind = db.prepare('SELECT 商品区分 FROM m_products WHERE 商品コード = ? COLLATE NOCASE').get(sku)?.商品区分;
    if (kind === 'セット') {
      recalcSetSalesClass(db, sku, now);
    } else {
      db.prepare('UPDATE m_products SET 売上分類 = NULL, updated_at = ? WHERE 商品コード = ? COLLATE NOCASE').run(now, sku);
    }
    // この商品を構成品に持つセットの導出値も更新する
    setsUpdated = refreshSetSalesClasses(db, [sku], now);
  });

  try {
    tx();
  } catch (e) {
    console.error('[sales_class] 削除の反映に失敗:', e.message);
    return res.status(500).json({ error: '売上分類削除の反映に失敗しました', detail: e.message });
  }
  res.json({ ok: true, deleted, sets_updated: setsUpdated });
});
...
// 単品の手動税率を登録・削除したときに、その単品を構成品に持つセット商品の
...
//
// 呼び出し側の責務: 単品側の更新と同一トランザクションで呼ぶこと。
//   ここだけ失敗して単品とセットの税率がねじれると、この修正が潰した不整合が再発する。
//   例外は握り潰さず呼び出し元まで投げる (トランザクションごとロールバックさせる)。
function collectParentSets(db, componentSkus) {
  const stmt = db.prepare(
    'SELECT DISTINCT セット商品コード FROM m_set_components WHERE 構成商品コード = ? COLLATE NOCASE'
  );
  const parents = new Set();
  for (const sku of componentSkus) {
    for (const r of stmt.all(sku)) {
      if (r.セット商品コード) parents.add(r.セット商品コード);
    }
  }
  return parents;
}

export function refreshSetTaxRates(db, componentSkus, now) {
  ...
  for (const setCode of parents) {
    const comps = getComponents.all(setCode).map(c => ({
      neTaxRate: c.ne_tax,
      manualTaxRate: c.manual_tax,
      componentExists: !!c.ne_exists,
    }));
...
// 売上分類を登録・削除したときに、その商品を構成品に持つセット商品の
// m_products.売上分類 も再計算する。算出ロジックは rebuild と同じ resolveSetSalesClass。
// 税率と同じく、rebuild (日次) 任せにすると「単品は登録済みなのにセットだけ未分類」が
...
export function recalcSetSalesClass(db, setCode, now) {
  ...
  const manual = db.prepare(
    'SELECT sales_class FROM product_sales_class WHERE sku = ? COLLATE NOCASE'
  ).get(setCode)?.sales_class;

  const comps = db.prepare(`
    SELECT sc.sales_class AS sales_class,
           p.商品コード IS NOT NULL AS ne_exists
    FROM m_set_components c
    LEFT JOIN raw_ne_products p ON c.構成商品コード = p.商品コード COLLATE NOCASE
    LEFT JOIN product_sales_class sc ON c.構成商品コード = sc.sku COLLATE NOCASE
    WHERE c.セット商品コード = ?
...
export function refreshSetSalesClasses(db, componentSkus, now) {
  ...
  if (parents.size === 0) return 0;
  let updated = 0;
  for (const setCode of parents) updated += recalcSetSalesClass(db, setCode, now);
  return updated;
}

// ─── DELETE /api/tax_rate/:sku ───
// 削除時は m_products の税率を「手動登録なし」で解決し直す。
// NE 側に有効な税率があればそれが残り、無ければ NULL/UNKNOWN で未登録一覧に再出現する。
...
router.delete('/api/tax_rate/:sku', (req, res) => {
  ...
  const tx = db.transaction(() => {
    ...
    deleted = db.prepare('DELETE FROM product_tax_rate WHERE sku = ?').run(sku).changes;
    ...
    const { taxRate, taxCategory } = resolveTaxRate(neRow?.消費税率, undefined);
    db.prepare('UPDATE m_products SET 消費税率 = ?, 税区分 = ?, updated_at = ? WHERE 商品コード = ? COLLATE NOCASE')
      .run(taxRate, taxCategory, now, sku);
    setsUpdated = refreshSetTaxRates(db, [sku], now);
  });

  try {
    tx();
  } catch (e) {
    console.error('[tax_rate] 削除の反映に失敗:', e.message);
    return res.status(500).json({ error: '税率削除の反映に失敗しました', detail: e.message });
  }
  res.json({ ok: true, deleted, sets_updated: setsUpdated });
});
  ...
  try {
    ...
    if (!type || type === 'sales_class') {
      rows = rows.concat(db.prepare(`
        ...
        -- セットも対象に含める (2026-08-07)。セットの売上分類は構成品から導出するが、
    ...
    if (!type || type === 'tax_rate') {
      rows = rows.concat(db.prepare(`
        SELECT 'tax_rate' as missing_type, m.商品コード, m.商品名, m.標準売価 as 売価, m.原価,
          m.取扱区分, m.商品区分,
          COALESCE(s7.qty, 0) as sales_7d,
          COALESCE(s30.qty, 0) as sales_30d,
          s30.last_sold,
        ...
        WHERE m.商品区分 IN ('単品', '例外') AND m.消費税率 IS NULL
        ORDER BY priority, sales_7d DESC, sales_30d DESC
        LIMIT 200
      `).all(cutoff7Str, cutoff30Str));
    }
...
router.post('/api/sales_class', (req, res) => {
  ...
  // 登録・監査ログ・m_products 反映・親セット再計算は 1 トランザクション。
  // 途中で失敗して「単品だけ登録済み・セットは未分類」になると、会計側の
  // 「その他/未分類」に落ちる元のバグが再発するため (refreshSetTaxRates と同じ理由)
  let setsUpdated = 0;
  const tx = db.transaction(() => {
    const old = db.prepare('SELECT * FROM product_sales_class WHERE sku = ?').get(sku);
    ...
    // m_productsにリアルタイム反映
    db.prepare('UPDATE m_products SET 売上分類 = ?, updated_at = ? WHERE 商品コード = ? COLLATE NOCASE').run(sc, now, sku);
    // この商品を構成品に持つセットの導出値も即時反映する (rebuild を待たない)
    setsUpdated = refreshSetSalesClasses(db, [sku], now);
  });
  ...
  try {
    ...
    console.error('[sales_class] 登録の反映に失敗:', e.message);
    return res.status(500).json({ error: '売上分類登録の反映に失敗しました', detail: e.message });
  }
  res.json({ ok: true, sku, sales_class: sc, sets_updated: setsUpdated });
});
...
router.post('/api/csv/sales_class', upload.single('file'), (req, res) => {
  ...
  let count = 0, skipped = 0, invalid = 0, setsUpdated = 0;
  const tx = db.transaction(() => {
    const touched = [];
    for (const row of dataRows) {
      const sku = (row[colSku] || '').toLowerCase().trim();
    ...
    // 取り込んだ商品を構成品に持つセットの導出値をまとめて更新する
    //   (行ごとに呼ぶと同じ親セットを何度も再計算するので、最後に1回だけ)
    setsUpdated = refreshSetSalesClasses(db, touched, now);
  });

  try {
    tx();
  } catch (e) {
    console.error('[sales_class] CSV取込の反映に失敗:', e.message);
    return res.status(500).json({ error: '売上分類CSV取込の反映に失敗しました', detail: e.message });
  }
  res.json({ ok: true, imported: count, skipped, invalid, total: dataRows.length, sets_updated: setsUpdated });
});
...
router.post('/api/tax_rate', (req, res) => {
  const sku = (req.body.sku || '').toLowerCase();
  ...
  // 単品の登録・監査ログ・m_products 反映・親セット再計算は 1 トランザクション。
  // 途中で失敗したときに単品だけ更新されてセットが取り残される (= 元のバグの再発) を防ぐ
  let setsUpdated = 0;
  const tx = db.transaction(() => {
    const old = db.prepare('SELECT * FROM product_tax_rate WHERE sku = ?').get(sku);
    ...
    db.prepare('UPDATE m_products SET 消費税率 = ?, 税区分 = ?, updated_at = ? WHERE 商品コード = ? COLLATE NOCASE').run(taxRate, taxCategory, now, sku);
  ...
  try {
    ...
    console.error('[tax_rate] 登録の反映に失敗:', e.message);
    return res.status(500).json({ error: '税率登録の反映に失敗しました', detail: e.message });
  }
...
router.post('/api/csv/tax_rate', upload.single('file'), (req, res) => {
  ...
  let count = 0, skipped = 0, invalid = 0, setSkipped = 0, unresolvedSkipped = 0, setsUpdated = 0;
  ...
  const tx = db.transaction(() => {
    for (const row of dataRows) {
      ...
      updateMp.run(taxRate, taxCategory, now, sku);
      touchedSkus.add(sku);
      count++;
    }
    // 取込んだ単品を構成品に持つセットの税率も同じトランザクション内で反映する。
    // 親セットは refreshSetTaxRates 内で重複排除されるので、同じセットは1回だけ再計算される
    setsUpdated = refreshSetTaxRates(db, touchedSkus, now);
  });

  try {
    tx();
  } catch (e) {
    console.error('[tax_rate/csv] 取込の反映に失敗:', e.message);
    return res.status(500).json({ error: '税率CSV取込の反映に失敗しました', detail: e.message });
  }
  res.json({ ok: true, imported: count, skipped, invalid, setSkipped, unresolvedSkipped, sets_updated: setsUpdated, total: dataRows.length });
});
...
router.get('/api/missing/download', (req, res) => {
  ...
  try {
    if (type === 'shipping') {
      ...
      rows = db.prepare(`
        SELECT m.商品コード, m.商品名, m.商品区分, m.取扱区分, m.標準売価, m.原価, m.原価状態, m.消費税率
        FROM m_products m
        WHERE m.商品区分 IN ('単品', '例外') AND m.消費税率 IS NULL
        ORDER BY m.取扱区分, m.商品コード
      `).all();
...
router.get('/api/missing/counts', (req, res) => {
  ...
  try {
    ...
    const sales_class = db.prepare("SELECT COUNT(*) as cnt FROM m_products WHERE 商品区分 IN ('単品', '例外', 'セット') AND 売上分類 IS NULL").get().cnt;
