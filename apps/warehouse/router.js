...
// ─── GET /api/cross-sell ───
// NE商品コードで検索 → 過去N日(既定90日)に同じ伝票で一緒に買われた商品ランキング。
// セット商品の構成品同士は伝票内に必ず同居してノイズになるため、raw_ne_set_products
// で兄弟構成品を除外する。集計対象モールは _ignore と amazon_fbm を除外
// (FBA は NE に入らない / amazon_fbm は要件で除外)。

// JST 基準で N 日前の YYYY-MM-DD を返す。
// toISOString は UTC なので、JST 0:30 に走らせると 1 日前の日付になりカットオフがズレる。
...
router.get('/api/cross-sell', (req, res) => {
  const code = (req.query.code || '').trim();
  const days = Math.max(1, Math.min(parseInt(req.query.days) || 90, 365));

  if (!code) return res.status(400).json({ error: 'code パラメータが必要です' });

  // 受注日 TEXT は YYYY-MM-DD 想定 (既存 /api/orders/daily と同じ前提)。
  const fromStr = jstDateString(days);

  // (商品コード × platform) 単位の中間集計を全件取る。LIMIT は掛けない。
  // 掛けると後段の global 再集計でランキングが欠落する (Codex R2 #1)。
  // 除外条件で対象は通常数百〜数千ペアに収まる。
  const sql = `
  ...
  // 対象伝票数: ランキング本体と同じ除外条件 (_ignore, amazon_fbm) を適用しないと
  // 「対象伝票数あり / 結果ゼロ」の見え方になる (Codex R2 #5)。
  const targetCount = execQuery(
  ...
  const productRows = execQuery(
    'SELECT 商品名 FROM raw_ne_products WHERE 商品コード = ?',
    [code]
  );
  const searchedProductName = productRows[0]?.商品名 || null;

  res.json({
    code,
    商品名: searchedProductName,
