...
// 衝突時に更新する列 (状態系 + 配送方法系 + 店舗コード)。
// 店舗コードは「識別系だから不変」に見えるが集計キーなので更新対象に入れる:
// 初回が空や仮値だったり NE 側で店舗帰属が直されたりすると、モール別件数が
// 恒久的に誤分類されたまま自己修復しない (Codex R1 high)。
const UPDATE_COLS = [
  '店舗コード', '出荷確定日', '受注状態区分', '受注状態', 'キャンセル区分', '受注キャンセル日',
  '配送方法ID', '配送方法名', '送り状番号',
];
