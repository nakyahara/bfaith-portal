#!/usr/bin/env node
/**
 * import-yahoo-data.js — Yahoo!ストクリ統計CSV incoming/ 取込 CLI (mall-csv-fetcher P1-Y)
 *
 * <DATA_DIR>/incoming/yahoo-data/ 直下の *.csv / *.json / *.zip を走査して warehouse.db へ UPSERT。
 * (1日単位型=商品分析/検索流入 は画面POSTのJSONが正規経路。他4種はCSV)
 * 対象: 全体分析/流入離脱/お客様分析/速報/商品分析/検索流入 の6種。種別はヘッダ自動判別。
 *
 * ⭐手動DLがこのフォルダに置くだけで即取込される (自動DLは後続スパイクで追加予定)。
 * 成功 → processed/YYYY-MM/、失敗 → failed/ へ移動 (削除しない)。sha256 冪等。
 *
 * 使い方:
 *   DATA_DIR=... node apps/warehouse/import-yahoo-data.js [--dry-run]
 *   node apps/warehouse/import-yahoo-data.js --data-dir C:\path\to\data
 *
...
const incomingDir = path.join(DATA_DIR, 'incoming', 'yahoo-data');
...
console.log(`=== Yahoo!統計CSV取込 (incoming: ${incomingDir}) ===`);
...
try {
  for (const name of entries) {
    ...
    const outcome = importYahooFile(db, { name, buffer, sha256, source: 'incoming' });
