...
function createTables() {
  ...
  // sort_order=0 一意制約 (Codex Round 17 指摘 3 対応): v_sku_components_first の「1 SKU = 1 行」を保証
  db.exec('CREATE UNIQUE INDEX IF NOT EXISTS idx_m_sku_components_first_unique ON m_sku_components(seller_sku) WHERE sort_order = 0');
  ...
  //   結果: 原価カバー 71.9% → 99.4%
  //   (Codex Round 17 指摘: 旧版は SKU 統合プロジェクトで上書きされた可能性あり、本 backport で復元)
  ...
  // (Codex Round 17 指摘: DDL 変更を repo に backport しないと initDB で巻き戻る → 本ブロックで対応)
  ...
  // 注意 (Codex Round 17 指摘):
  //   現在 LEFT JOIN v_sku_costed c は component grain なので、複数構成品セット SKU で行が複製される。
  //   cogs (SUM) は問題ないが、wide の他の集計列を SELECT で出すと重複する。
  //   将来 cogs_by_sku_month サブクエリで先に集計してから 1:1 JOIN に変更予定 (Phase 3.1.y で対応)
  ...
  db.exec(`CREATE VIEW v_amazon_sku_profit_actual_v4 AS
    ...
    SELECT
      ...
      -- gross margin (販売数量ベース、広告費除く)
      -- 注: commission/fba/promotion 等は raw 負値 → + で実質減算
      ...
      -- settlement margin (refund/補償/広告費 全部反映)
      (w.sales_principal_micro / 1000000.0)
        - COALESCE(c.unit_cogs_excl_tax * w.qty_ordered, 0)
        + (w.commission_micro / 1000000.0)
        + (w.fba_fulfillment_micro / 1000000.0)
