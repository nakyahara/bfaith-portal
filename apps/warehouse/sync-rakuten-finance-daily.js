#!/usr/bin/env node
/**
 * sync-rakuten-finance-daily.js — 楽天 Phase 1a #R-3 entity-driven sync
 *
 * f_rakuten_finance_sku_daily_v1 → mirror_rakuten_finance_sku_daily に sync する
 * contract-driven 実装。Amazon Phase 1 #1-4 (sync-amazon-finance-daily.js) と同型。
 *
 * 設計:
 *   - sync_contracts に登録された contract version で送信
 *   - first chunk で `meta.clear_rakuten_finance_dates` を送って Render 側で DELETE
 *   - INSERT OR REPLACE on PK (date_jst, rakuten_code) で同一 run の再送は idempotent
 *   - source_run_id / source_row_hash で audit trail
 *   - dry-run / scoped sync (--from / --to / --month) サポート
 *   - DATA_DIR 必須 (fail-fast)
 *
 * 受信側 (Render の warehouse-mirror/router.js) は #R-3b で実装。
 * daily-sync.js 統合は #R-3c で実装。
 *
 * 使い方:
 *   DATA_DIR=... node apps/warehouse/sync-rakuten-finance-daily.js --month 2026-04 --dry-run
 *   DATA_DIR=... node apps/warehouse/sync-rakuten-finance-daily.js --month 2026-04
 *
 * env:
 *   DATA_DIR             必須
 ...
 *   1: contract mismatch / send error
 *   2: env error
 */
...
const ENTITY_NAME = 'rakuten_finance_sku_daily';
...
if (!contract) {
  ...
  console.error(`  Run: sqlite3 warehouse.db < sql/sync/seed_rakuten_finance_sku_daily.sql`);
  process.exit(1);
}
...
const rows = db.prepare(`
  SELECT * FROM f_rakuten_finance_sku_daily_v1
  WHERE date_jst >= ? AND date_jst <= ?
  ORDER BY date_jst, rakuten_code
...
// run_id 生成 (秒+ms+乱数 6 hex で同分内 PK 衝突回避)
...
const enrichedRows = rows.map(r => {
  ...
  const hashInput = JSON.stringify({
    ...
    units_cancelled: r.units_cancelled,
  });
...
import('node:os').then(async (os) => {
  ...
  if (isDryRun) {
    ...
    console.log(`  First chunk meta: clear_rakuten_finance_dates=[${distinctDates.length} dates]`);
    console.log(`  Sample row keys: ${Object.keys(enrichedRows[0]).join(', ')}`);
    const r0 = enrichedRows[0];
    console.log(`  Sample row[0]: date=${r0.date_jst} rakuten_code=${r0.rakuten_code} ne_code=${r0.ne_code} margin=${r0.variable_margin_jpy_incl} hash=${r0.source_row_hash}`);
    process.exit(0);
  }
  ...
  for (let i = 0; i < chunks.length; i++) {
    ...
    const body = {
      ...
      meta: isFirst ? { clear_rakuten_finance_dates: distinctDates } : {},
      payload,
    };
    ...
    try {
      ...
      if (!res.ok) {
        ...
        // 409 incomplete (is_last 受信後の欠番検出) は当該 chunk 自体は DB 反映済み
  ...
  if (lastError) {
    ...
    // 3d. Render mart rebuild trigger (Phase 1a 段階では noop endpoint 想定)
    try {
