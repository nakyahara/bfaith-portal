...
export async function rebuildFSales() {
  ...
  // SKU解決ソース: v_sku_resolved (master only、Step 4-0 で sku_map fallback 撤廃済)
  console.log(`[f_sales] SKU解決ソース: v_sku_resolved (master only)`);
  ...
  // SKU解決マップ（seller_sku → [{ne_code, 数量}]）、ソース: v_sku_resolved (master only)
  ...
  // 1. v_sku_resolved (master only) → 2. m_products.商品コードと直接一致 → 3. unmapped
  function resolveAmazonSku(sku) {
    const mapped = skuMap.get(sku);
    if (mapped) return { mappings: mapped, source: 'resolved' };
  ...
  for (const row of amazonRows) {
    ...
    if (!resolved) {
      unmappedRows.push({
        ...
        reason: 'SKU未解決 (m_sku_master未登録) ・商品コード不一致',
      });
      unmappedCount++;
      continue;
    }
  ...
  log.push(`Amazon直接マッチ（v_sku_resolved不要）: ${directMatchCount}件`);
