#!/usr/bin/env node
/**
 * sync-qoo10-finance-daily.js — Qoo10 Phase 1 A-3 entity-driven sync
 *
 * f_qoo10_finance_sku_daily_v1 → mirror_qoo10_finance_sku_daily に sync。
 * LINEギフト Phase 1 A-3 (sync-linegift-finance-daily.js) 同型。
 *
 * 設計書: g:/共有ドライブ/AI_reference/システム設計/Qoo10Phase1設計書_v0.11_20260518.md §9
 *
 * 使い方:
 *   DATA_DIR=... node apps/warehouse/sync-qoo10-finance-daily.js --month 2026-05 --dry-run
 *   DATA_DIR=... node apps/warehouse/sync-qoo10-finance-daily.js --month 2026-05
 *
 * env:
 *   DATA_DIR             必須
...
const ENTITY_NAME = 'qoo10_finance_sku_daily';
const CONTRACT_VERSION = 1;
// Qoo10 fact は LINEギフト (~40 列) より少し多い (~55 列、promo/scope/domestic_non_cod 別集計あり)。
// LINEギフト/au PAY と同じ 500 行 chunk で運用 (Qoo10 は月 ~300 行レベルで CHUNK 1-2 個想定)。
...
if (monthStr) {
  ...
  // Codex (A-3) Medium #1 + R2 Medium #1 反映: --from/--to の YYYY-MM-DD 形式 + 実在日付 + from<=to チェック
  // 形式 OK だが '2026-02-31' のような非実在日付を弾くため Date parse 後の再フォーマット一致を確認
  ...
  if (!isRealDate(fromStr) || !isRealDate(toStr)) {
    console.error(`FATAL: --from/--to must be real YYYY-MM-DD date (got from=${fromStr}, to=${toStr})`);
    process.exit(2);
  }
  if (fromStr > toStr) {
    console.error(`FATAL: --from (${fromStr}) must be <= --to (${toStr})`);
    process.exit(2);
  }
...
const rows = db.prepare(`
  SELECT * FROM f_qoo10_finance_sku_daily_v1
...
// clear させる必要がある (LINEギフト/au PAY 同型)。

// clear_qoo10_finance_dates は scope 全日付を列挙する (rows.map(r=>r.date_jst) だと「その日が source で
...
const enrichedRows = rows.map(r => {
  // Qoo10 特有のキー (match_tier / promo / scope / cogs / margin / shipping) を hash に含める
  // Codex (A-3) High #1 反映: promo 金額列 (件数だけでなく) も hash 入力に追加、件数不変で金額のみ変わる修正も差分追跡
  const hashInput = JSON.stringify({
    date_jst: r.date_jst,
    sku_code: r.sku_code,
    ne_code: r.ne_code || '',
    match_tier: r.match_tier || '',
    settle_price_formula_scope: r.settle_price_formula_scope || '',
    net_settlement_api_jpy_incl: r.net_settlement_api_jpy_incl,
    variable_margin_jpy_incl: r.variable_margin_jpy_incl,
    cogs_amount_jpy_incl: r.cogs_amount_jpy_incl,
    shipping_cost_jpy_incl: r.shipping_cost_jpy_incl,
    units_ordered: r.units_ordered,
    megawari_order_count: r.megawari_order_count,
    megawari_discount_amount_jpy_incl: r.megawari_discount_amount_jpy_incl,
    megapo_discount_amount_jpy_incl: r.megapo_discount_amount_jpy_incl,
    other_promo_discount_jpy_incl: r.other_promo_discount_jpy_incl,
    total_platform_promo_jpy_incl: r.total_platform_promo_jpy_incl,
    shop_promo_burden_jpy_incl: r.shop_promo_burden_jpy_incl,
...
import('node:os').then(async (os) => {
  ...
  if (isDryRun) {
    ...
    console.log(`  First chunk meta: clear_qoo10_finance_dates=[${distinctDates.length} dates]`);
    if (enrichedRows.length > 0) {
      console.log(`  Sample row keys: ${Object.keys(enrichedRows[0]).join(', ')}`);
      const r0 = enrichedRows[0];
      console.log(`  Sample row[0]: date=${r0.date_jst} sku_code=${r0.sku_code} ne_code=${r0.ne_code} tier=${r0.match_tier} margin=${r0.variable_margin_jpy_incl} shipping=${r0.shipping_cost_jpy_incl} hash=${r0.source_row_hash}`);
    } else {
  ...
  for (let i = 0; i < chunks.length; i++) {
    ...
    const body = {
      ...
      meta: isFirst ? { clear_qoo10_finance_dates: distinctDates } : {},
      payload,
    };
