...
// 過去データ一括インポート（APIキー認証 or セッション認証、未設定時はスキップ）
router.post('/import-historical', (req, res) => {
  ...
  // key未設定なら認証スキップ（warehouse-mirror と同じポリシー）
  if (key && !sessionOK && provided !== key) return res.status(401).json({ error: 'Invalid sync key' });
