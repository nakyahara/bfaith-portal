 ...
 *   ③ 締めた日には raw の整理 (raw.purge_superseded_observations 30 日 = D-25) と DB の大きさの記録 (ops.job_runs)。
 *
 * 約束:
 *   - 世代 (取得時刻) = mirror の captured_at (miniPC が CSV を取った時刻)。observed_at にもこれを使う (在庫日ではない。R3 #5)。
 *     ops.ingest_runs.checksum に世代を ISO で残す (「前回と同じ世代か」「その日の最後の世代はどれか」の根拠)
 *   - 比較元 = 直前までの **完走した run の状態観測 (ok / not_found)** だけ (失敗した run・error / skipped は根拠にしない。R3 #1 = view と同じ)
 *   - 同時実行は advisory lock (取引ロック) で直列化。表のロック順は contents → observations (整理と同じ順 = deadlock を避ける。R3 #10)
...
export async function captureLogizardInventory(db, { rows, capturedAt, host = 'render', runId = newRunId('inv'), companyId = COMPANY_ID, warehouseId = WAREHOUSE_ID, scope = SCOPE, log = () => {} } = {}) {
  ...
  // 世代の比較 (直前の完走した run)。同じ・古い世代は skipped の run だけ残す (観測は書かない)
  const last = await one(`select ingest_run_id, checksum from ops.ingest_runs where source_system = $1 and entity = $2 and scope_key = $3 and status = 'success' and complete and checksum is not null order by checksum desc limit 1`, [SOURCE, ENTITY, scope]);
  if (last && generation <= last.checksum) {
    const reason = generation === last.checksum ? `同じ世代 (${generation}) = 取り込み済み` : `古い世代 (${generation} < ${last.checksum})`;
    await db.query(`insert into ops.ingest_runs (ingest_run_id, source_system, entity, scope_key, host, started_at, finished_at, status, complete, rows_seen, rows_inserted, source_tz, checksum, format_version, error)
                    values ($1, $2, $3, $4, $5, now(), now(), 'skipped', false, $6, 0, 'UTC', $7, 'v1', $8)`, [runId, SOURCE, ENTITY, scope, host, rows.length, generation, reason]);
    log(`skipped: ${reason}`);
    return { status: 'skipped', runId, generation, seen: rows.length, added: 0, changed: 0, removed: 0, contentsNew: 0, locationsAdded: 0, reason };
  }
  ...
  try {
    ...
    if (!got) throw Object.assign(new Error('別の取込が走っている (advisory lock)'), { code: 'LOCKED' });
    ...
    if (newContents.size) {
      ...
      contentsNew = r.rowCount ?? r.affectedRows ?? 0;
    }
    if (obsKeys.length) {
      await db.query(
 ...
 * 戻り値 = { day, status: 'complete'|'missing'|'exists', runId, generation, lines, skus }
 */
export async function closeStockDay(db, day, { companyId = COMPANY_ID, warehouseId = WAREHOUSE_ID, scope = SCOPE, log = () => {} } = {}) {
  ...
  try {
    if (!run) {
      ...
      return { day, status: 'missing', runId: null, generation: null, lines: 0, skus: 0 };
    }
    ...
    // その世代までの最新の状態観測 (完走 run・ok / not_found) のうち ok の行 = その時点の在庫
    const lines = await db.query(
      `insert into snapshots.warehouse_stock_daily (snapshot_date, source, scope_key, line_key, company_id, sku_id, logizard_code, location_id, location_code, block_code, quality, expiry_date, received_date, qty, allocated_qty, captured_at, ingest_run_id)
       with state as (
         select distinct on (o.business_key) o.business_key, o.fetch_status, o.content_hash
           from raw.${RAW_SRC}_observations o join ops.ingest_runs r on r.ingest_run_id = o.ingest_run_id
          where o.scope_key = $3 and r.status = 'success' and r.complete and o.fetch_status in ('ok', 'not_found') and o.observed_at <= $6::timestamptz
          order by o.business_key, o.observed_at desc, o.observation_id desc),
       rows_ as (
         select st.business_key, c.payload as p,
                coalesce(nullif(trim(c.payload ->> 'ブロック略称'), ''), '-') || '-' || coalesce(nullif(trim(c.payload ->> 'ロケ'), ''), '-') as loc_code
           from state st join raw.${RAW_SRC}_contents c on c.content_hash = st.content_hash
          where st.fetch_status = 'ok')
       select $1::date, $2, $3, r.business_key, $4::smallint, k.sku_id, r.p ->> '商品ID', l.location_id, r.loc_code, r.p ->> 'ブロック略称', r.p ->> '品質区分名',
              case when (r.p ->> '有効期限') ~ '^\\d{4}[-/]\\d{1,2}[-/]\\d{1,2}' then to_date(replace(substring(r.p ->> '有効期限' from '^\\d{4}[-/]\\d{1,2}[-/]\\d{1,2}'), '/', '-'), 'YYYY-MM-DD') end,
              case when (r.p ->> '入荷日') ~ '^\\d{4}[-/]\\d{1,2}[-/]\\d{1,2}' then to_date(replace(substring(r.p ->> '入荷日' from '^\\d{4}[-/]\\d{1,2}[-/]\\d{1,2}'), '/', '-'), 'YYYY-MM-DD') end,
              (r.p ->> '在庫数')::integer, coalesce((r.p ->> '引当数')::integer, 0), $6::timestamptz, $5
         from rows_ r
         left join core.skus k on k.company_id = $4::smallint and k.code_norm = core.norm_code(r.p ->> '商品ID')
         left join core.locations l on l.warehouse_id = $7::smallint and l.code = r.loc_code`,
      [day, SOURCE, scope, companyId, run.ingest_run_id, run.generation, warehouseId]);
    ...
    await db.exec('commit');
    const out = { day, status: 'complete', runId: run.ingest_run_id, generation: run.generation, lines: lines.rowCount ?? lines.affectedRows ?? 0, skus: skus.rowCount ?? skus.affectedRows ?? 0 };
    log(`${day}: complete (世代 ${run.generation}, ロケ行 ${out.lines}, SKU ${out.skus})`);
    return out;
  } catch (e) {
    await rollbackQuiet(db);
...
/**
 * ② 未締めの日を全部締める (最初の観測日 〜 昨日。今日は締めない)。戻り値 = { closed: [closeStockDay の戻り値...], firstDay, upTo }
...
export async function closeStockDays(db, { todayJst = jstDateStr(new Date()), maxDays = 60, ...rest } = {}) {
  ...
  const first = (await one(`select min(observed_at) as t from raw.${RAW_SRC}_observations where scope_key = $1`, [scope]))?.t;
  if (!first) return { closed: [], firstDay: null, upTo: todayJst };
  ...
  const closed = [];
  for (let d = firstDay; d < todayJst && closed.length < maxDays; d = nextDay(d)) {
    if (done.has(d)) continue;
    closed.push(await closeStockDay(db, d, rest));
  }
  return { closed, firstDay, upTo: todayJst };
}
