...
/** W11 が読む注文 (自社発送・注文日 from〜to・取消でない) のうち候補だけ: A = モールで出荷済み & 伝票なし / B・B2 = 未発送アラートの無いモールで未発送の状態 ($6 = その状態の一覧。無ければ空) */
const W11_ROWS = `
  with o as (
    select o.order_id, o.mall_order_no, o.order_date_jst, o.status
      from core.orders o
  ...
  j as (
    select o.*, x.n_slips, x.has_shipped
      from o cross join lateral (select count(*)::int as n_slips, coalesce(bool_or(not s.is_cancelled and s.ship_date_jst is not null), false) as has_shipped
                                   from core.shipments s where s.order_id = o.order_id) x)
  select j.order_id::text as order_id, j.mall_order_no, j.order_date_jst::text as d, j.status, j.n_slips, j.has_shipped,
    ...
    from j
   where (j.n_slips = 0 and j.status in ('shipped', 'delivered', 'returned')) or j.status = any($6::text[])
...
export async function evalW11(ctx, check) {
  ...
  for (const m of config.ORDER_MALLS) {
    ...
    const r = base(check, scopeKey, { periodFrom: from, periodTo: to, severity: asOf < config.W11_INFO_UNTIL ? 'info' : check.severity,
      threshold: { a: 0, b: 0, b2: 0, lag_days: config.W11_LAG_DAYS, window_days: config.W11_WINDOW_DAYS, unshipped_statuses: u ? u.notShipped : [], status_fresh_days: u ? u.statusFreshDays : null } });
    ...
    const items = [];
    for (const x of rows) {
      const age = Math.round((Date.parse(`${asOf}T00:00:00Z`) - Date.parse(`${x.d}T00:00:00Z`)) / 86400000);
      let kind = null;
      if (Number(x.n_slips) === 0 && ['shipped', 'delivered', 'returned'].includes(x.status)) kind = x.slip_not_linked ? 'A_slip_not_linked' : 'A_no_slip';
      else if (u && u.notShipped.includes(x.status)) {
        if (!x.has_shipped) kind = 'B_unshipped';
        else if (u.statusFreshDays == null || age <= u.statusFreshDays) kind = 'B2_mall_not_notified';   // 状態が固定された後の注文は「モールが未発送」と言えない
      }
      if (kind) items.push({ subjectType: 'order', subjectKey: x.order_id, payload: { kind, mall_order_no: x.mall_order_no, order_date: x.d, status: x.status, age_days: age, ne_slips: Number(x.n_slips), weight: age } });
    }
    ...
    r.observed = { from, to, a: count('A_'), a_slip_not_linked: count('A_slip_not_linked'), b: count('B_'), b2: count('B2_'), unshipped_mall: !!u, oldest: items.length ? items[0].payload.order_date : null };
    r.inputGeneration = { from, to, shipments_run_id: evidence.shipments ? evidence.shipments.run_id ?? null : null };
    r.verdict = items.length ? 'breach' : 'pass';
    if (items.length) {
      const parts = [];
      if (r.observed.a) parts.push(`モールで出荷済みなのに NE の伝票なし ${r.observed.a}${r.observed.a_slip_not_linked ? ` (うち番号の合う伝票が結べていない ${r.observed.a_slip_not_linked})` : ''}`);
      if (r.observed.b) parts.push(`モールでも NE でも未発送 ${r.observed.b}`);
      if (r.observed.b2) parts.push(`NE で出荷済みなのにモールが未発送 ${r.observed.b2}`);
      r.reason = `${parts.join(' / ')} (注文日 ${from}〜${to}。最古 ${r.observed.oldest})`;
    }
    out.push(r);
  }
  return out;
...
export async function generationOf(db, config, asOf, { evidence = {} } = {}) {
  ...
  if (config.W11_WINDOW_DAYS) {
    ...
    for (const m of config.ORDER_MALLS) {
      const u = w11Unshipped(config, m.mall);
      w11.push(await part(`select count(*)::int as n, coalesce(sum(hashtext(x.order_id || ':' || x.status || ':' || x.n_slips || ':' || x.has_shipped || ':' || x.slip_not_linked)), 0)::bigint as h from (${W11_ROWS}) x`,
