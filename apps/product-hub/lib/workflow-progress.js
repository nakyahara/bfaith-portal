...
export function composeStateOf(summary, composeDoneAt) {
  ...
  return { excluded: false, done: marked || implied, marked, implied: !marked && implied };
}
...
export function boardData(db, { view = 'main', assigneeId = null, unassignedOnly = false, checkingOnly = false, imageKind = null, limit = 800, mallSummary = null, reconcileSet = null } = {}) {
  ...
  const drafts = db.prepare(`
    SELECT d.id, d.ne_code, d.name, d.status, d.created_at, d.updated_at, d.detail_images_excluded, d.image_priority, d.own_brand,
      ...
      (SELECT compose_done_at FROM draft_image_production ip WHERE ip.draft_id = d.id) AS compose_done_at,
  ...
  const imageSummaryOf = (p, d) => ({
    ...
    compose: composeStateOf(p.imageDetail, d.compose_done_at),
  });
