...
// 旧URLの ?status= 付きブックマーク・共有リンクは一覧へ引き継ぐ
router.get('/', (req, res) => {
  if (req.query.status !== undefined) {
    const q = DRAFT_STATUSES.includes(req.query.status) ? `?status=${encodeURIComponent(req.query.status)}` : '';
    return res.redirect(`/apps/product-hub/list${q}`);
  }
...
router.post('/api/drafts/:id/image-production', (req, res) => {
  ...
  // 対象商品は絞らない (2026-09-13 スタッフ要望: 仕入れ低の商品も簡易LPを作る。
  // 2026-08-31 までは自社商品 + 取扱先限定商品だけだった)
  const db = getDB();
  const b = req.body || {};
...
router.post('/api/drafts/:id/image-hold', (req, res) => {
  const draft = loadDraftOr404(req, res);
  if (!draft) return;
  // 画像制作情報 (image-production) と同じく全商品で使える (2026-09-13〜。画像制作カードを全商品に出したため)
  if (!canEditImageProduction(req)) {
...
router.post('/api/drafts/:id/rakuten', (req, res) => {
  ...
  // カタログID (JAN) 本体はここでは受けない (2026-09-02 中原さん: 入力は基本情報タブだけ)。
  // 単品は POST /api/drafts/:id の jan_code、バリエーションは /sku-jans が入口
