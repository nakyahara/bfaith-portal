...
- **2026-09-10 の作り直し**: `ap_policies` の `mode` CHECK に `custom` を足し `custom_type_id` を追加した。SQLite は CHECK を ALTER できないので、
  ...
  view は無い表を参照したまま残ると以後の DDL を壊すので先に消し、**トリガと view の復元まで同じトランザクションの中で**行う。さらに `createTables` の DDL 全体 (トリガの DROP → CREATE を含む) も 1 つの immediate トランザクション (途中で止まっても「トリガの無い表」は残らない)。
