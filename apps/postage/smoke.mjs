...
function makeWarehouse(file) {
  ...
  d.transaction(() => {
    ...
    slip('5', '2026-06-03', [['sku-c', 1]]);              // 厚み未登録 (列ズレで取り込まれない) → missing_thickness
...
t('白プチ・白ビ袋の外寸は未測定のまま (推測で埋めない)', () => {
  const rows = getDB().prepare("SELECT material_code, outer_length_mm FROM pm_materials WHERE material_code IN ('shiropuchi','shirobi')").all();
  eq(rows.every((r) => r.outer_length_mm === null), true);
});
...
t('内容が同じ重複は「注意」、違う重複は「要修正」', () => {
  ...
  eq(dup.some((i) => i.sku_code === 'sku-b' && i.severity === 'error'), true, '内容違い');
});
...
t('内容が違う重複は後の行で上書きされない', () => {
  eq(getDB().prepare("SELECT unit_weight_g w FROM pm_skus WHERE sku_code='sku-b'").get().w, 30, '99 で上書きされていない');
});
t('列ズレ行の厚みは取り込まない (誤った値を入れない)', () => {
  eq(getDB().prepare("SELECT thickness_mm t FROM pm_skus WHERE sku_code='sku-c'").get().t, null);
});
t('重さが読めなかった行は NULL のまま (0 で埋めない)', () => {
  eq(getDB().prepare("SELECT unit_weight_g w FROM pm_skus WHERE sku_code='sku-d'").get().w, null);
});
...
t('定形外だけを対象にする (ネコポスは数えない)', () => { eq(rep.total, 7); });
t('茶封筒・薄い・軽い → 定形110円で確定', () => {
  eq(rep.confirmed, 2);
...
t('「この資材を測ると何通動くか」が出る', () => {
  const b = rep.blockedMaterials.find((m) => m.material_code === 'shiropuchi');
  eq(b?.count, 1);
});
t('不足SKUが出現回数の多い順に並ぶ', () => {
  eq(rep.missingSkus.length > 0, true);
  const c = rep.missingSkus.map((m) => m.count);
  eq(c.every((v, i) => i === 0 || c[i - 1] >= v), true, '降順');
});
t('マスタに無いSKUも不足として出る', () => {
  eq(rep.missingSkus.some((m) => m.sku_code === 'sku-zzz'), true);
});
