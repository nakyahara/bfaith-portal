...
- 🚨 **待ち時間は外側ほど長くする**: miniPC 最大240秒 (ロック60 + 取得180) < Render 300秒 < 管理画面 330秒。
...
- 🚨 **「増えていなかった」と言ってよいのは、取り直した結果が届いたと確かめられたときだけ**。
  裏取りは ①Drive の更新日時が動いた ②miniPC の出力CSVが前回と同じ (= 増えていないことが miniPC 側で確定) のどちらか。
  どちらも取れなければ `not_verified` で「共有ドライブへの反映を確認できませんでした」と返す
