...
- 🚨 **繰り越しは「取込が来ている」ことの保証ではない**。取得が壊れていても前日の一覧で作業は続いてしまうので、
  **「新しい受付が増えていないだけ」(正常) と「そもそも取りに行けていない」(要調査) を言い分ける** (Codex #1231 R1)。
  ...
  - 今日読めている → 🔵「9/05 の一覧を引き継いで作業中。本日 10:12 時点で**新しい入荷受付は増えていません**」
    (管理画面では、Drive の CSV 自体が古いままなら「rclone は中身が同じなら転送しないのでこれ自体は異常ではないが、
    定時取得が動いているかは jobs-monitor で確認」を併記する)
