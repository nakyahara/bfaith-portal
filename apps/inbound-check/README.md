...
| `/apps/inbound-check/camera-test` | **認証なし** | 📷 カメラの確認。**アプリの読み取り機能を一切通さず** getUserMedia を1回だけ呼んで10秒観察する。端末側かアプリ側かの切り分け用 (2026-09-07 追加) |
...
- 📷 カメラ = **zxing-wasm で自前でデコードする**。読んだ値を検索欄に入れて検索するだけ
  ...
  - 🚨 **iPad だけ「掴めたのに即 ended」になる** (2026-09-07 実機。iPhone は同じコードで通った)。
    ...
    - 📌 **切り分けの基準点** = `/apps/inbound-check/camera-test` (認証なし)。
      **アプリのロジックを一切通さない**最小のページ。`getUserMedia({video:true})` を1回呼び、
      `ended` / `mute` / コマの進みを10秒ぶん出す。ここで映れば端末は問題なし、
      映らなければ端末側。**何かを足すとこの切り分けが濁る**ので、機能を増やさないこと
      (`scripts/test-inbound-check-camera-test-page.mjs` がそれを守る)
