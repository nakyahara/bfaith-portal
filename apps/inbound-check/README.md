...
- 🎯🎯 **原因確定 (2026-09-07): iPad × ホーム画面アプリ (standalone PWA) で iPadOS がカメラを即手放す**

  **アプリのロジックを一切通さない最小ページ** (`/camera-test`) で確定させた:

  | 端末 / 開き方 | 結果 |
  |---|---|
  | iPhone・ホーム画面アプリ | ✅ 読める |
  | iPad・**Safari** | ✅ 10秒間 live (`user 640x480`・進み19回) |
  | iPad・**ホーム画面アプリ** | ❌ **20ms で ended** (同じページ・同じカメラ・同じ条件) |

  全画面 (窓768x1004 / 画面768x1024)・`表示=visible`・分割なし。`ended` は**イベント**で観測。
  → **読み取り画面の作りは無関係**。WebKit に同種の報告あり (bugs.webkit.org #252465 / #264288)
  だが、入れ直し・更新で直らなかった報告もあるので同一原因とは断定しない。

  ⭐**打つ手 (アプリ側の修正では直らない)**:
  - その iPad では **Safari で使う** (動作確認済み。⚠Safari と PWA は Cookie 領域が別 = 端末登録が別途必要)
  - **検索欄を長押し →「テキストをスキャン」** (iOS 標準 Live Text。カメラ API を使わないので影響を受けない)
  - 復旧検証は **アプリ完全終了 → iPad 再起動 → ホーム画面アプリ入れ直し → iPadOS 更新** を**一つずつ**
  - (未検証) manifest の `display` を `standalone` 以外にすると回避できる可能性

  🚨🚨 **この件の教訓**: 「直すかもしれない修正」を先に**3回**出して**3回とも外した**
  (#1235 起動順・#1239 条件緩和・#1241 画面分割)。どれも実在の不具合ではあったが原因ではなかった。
  **切り分けの道具 (アプリのロジックを通さない最小ページ) を先に作るべきだった。**
  実機を触れないときほど、当てにいく前に「何が効いているか分かる状態」を作る。

- 参考 (確定前の観測): 「iPad × ホーム画面アプリ」の組み合わせだけカメラが 9ms で切れる
  | | 結果 |
  |---|---|
  | iPhone・ホーム画面アプリ | ✅ 動く |
