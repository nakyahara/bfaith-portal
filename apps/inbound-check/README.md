...
- 🚨🚨 **2026-09-07 実測: 「iPad × ホーム画面アプリ」の組み合わせだけカメラが 9ms で切れる**
  ...
  **推論**: ブラウザ / OS 側のキャプチャ終了を疑う。**誘因は未確定** — ページ側の操作が
  ...
  - 🚨 **iPad はカメラを掴んだ直後に一度 `muted` になり、約0.9秒後に `unmute` で復帰する**
    (2026-09-07 実測。iPhone では出ていない)。`muted` の間は `videoWidth` は入るのに
    `readyState=0` でコマが進まない = **「掴めた」だけでは映っていない**証拠がまた1つ増えた。
    見張りが `track.muted` を「届いていない」と数えるのは、この癖に対しても正しい
