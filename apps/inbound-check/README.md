...
**Notion が正本だった時期にしか作られない** (`f_inbound_check_destinations.notion_page_id` を書くのは `notion-sync.js` だけで、
Notion 廃止後は取込の口が 410 で閉じている) ので、起き得ない場合のために入荷側の状態がカードへ書き込む口を
