...
【自動 (通常)】miniPC Logizard-NyukaCSV (毎日 00:20 / 08:40 / 11:45)
  ...
  → Render が30分おきに取得 (JST 0〜20時台。同じ内容なら取り込まない)
...
- Render の巡回を `*/30 6-20` → `*/30 0-20` に広げて **00:30 に取り込む** (6時始まりのままだと反映が朝まで待ち)
- 🚨 **dead-man の anchor は 08:40 のまま**。00:20 に寄せると「深夜が成功していれば朝が壊れていても緑」に
  なり、現場が使う時間帯の故障を隠す。深夜は「あれば嬉しい」ぶんで、正は朝
