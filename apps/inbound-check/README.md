...
- 📷 カメラ = **zxing-wasm で自前でデコードする**。読んだ値を検索欄に入れて検索するだけ
  ...
  - 🚨 **映像が1コマも来ないまま8秒たったら打ち切る** (`MAX_IDLE_STREAK`)。
    黒い画面のまま案内だけ出続けると、現場は正常との区別がつかない。
    打ち切りの理由 (`no_video` / `decode`) で文言を出し分ける — 打つ手が違うため
  ...
  - テスト = `node scripts/test-inbound-check-barcode-scan.mjs` (53項目)。実機を持たずに「読める」と
    言わないための下限:
    ...
    - **安全弁の挙動** (2フレーム一致 / A→例外→A で確定しない / 8回で打ち切り / 映像が来なければ
      8秒で打ち切り / FNSKU も通る)
