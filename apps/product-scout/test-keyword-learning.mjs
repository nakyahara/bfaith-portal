...
import {createKeywordTables,ingestKeywords,keywordQueue,keywordSyncState,recordKeywordDecision,keywordHistory,latestKeywords,REASONS,REASON_GROUPS} from './keywords.js';
...
test('翌日の案が増えても未判定を保持。理由をワンクリックで保存し、判断の訂正を返す',async t=>{
 ...
 const html=await ejs.renderFile(fileURLToPath(new URL('./views/keyword-queue.ejs',import.meta.url)),{run:latestKeywords(db),today:'2099-01-01',queue:keywordQueue({},db),reasonLabels:REASONS,reasonGroups:REASON_GROUPS});assert.ok(html.includes('未判定の案は翌日も残ります'));assert.ok(html.includes('用途1 シート'));assert.ok(html.includes('保存時の参考価格'));
});
