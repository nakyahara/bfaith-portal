...
router.post('/api/listings', async (req, res) => {
  try {
    ...
    try {
      ...
      // SKU解決マップ: seller_sku → ne_code (mirror_sku_resolved master only)
      const skuMappings = db.prepare('SELECT seller_sku, ne_code, quantity AS 数量 FROM mirror_sku_resolved').all();
