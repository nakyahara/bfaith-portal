...
/**
 * 🚨**アプリからの呼び出しは 2026-09-09 に全部無くなった** (呼ぶのはテストだけ)。
 *   中原さん 2026-09-09:「いろはのアプリには新規カードを作るだけ。削除は絶対にダメ」。
 *   入荷側 (取込・やり直し) とカードの統合から、この関数を呼ぶ口をすべて外した。
 *   **入荷側の都合でカードに書く経路を、ここに新しく繋がないこと。**
 *   残してあるのは、2026-09-08 に付いた札がまだ残っているカードを職員が片づける道 (resolveCancellation)
 *   を動かし続けるため + その動きをテストで固定するため。
 *
