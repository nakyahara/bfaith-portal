 ...
 *     (出荷Noは毎日再利用されるため)
 *
 * ⑤梱包対応 (2026-08-18): 本体を createNotionCardSync (factory) に一般化し、
 * picking はその1インスタンス。梱包版 = apps/packing/notion.js (要件§5.10 の共通部品化)。
 */

const NOTION_VERSION = '2022-06-28';
const TIMEOUT_MS = 8000;
const SCHEMA_TTL_MS = 10 * 60 * 1000;
export const DAILY_DB_ID = process.env.PICKING_NOTION_DB || 'c87194a151f44108a21a5514c932c27b';
...
/**
 * デイリー業務カード同期の factory。picking / packing で工程別の設定を渡して共用する。
 * @param config {
 *   dbId: string,
 *   tokenEnvs: string[],      // 優先順の env 名。最初に値があるものを使う (未設定=連携オフ)
 *   logPrefix: string,
 *   workerPropName: string,   // 担当者 select のプロパティ名 (normName で照合)
 *   optionalProps: { startProp?, endProp?, minutesProp?, secPerLineProp? } — {name, type}
 * }
 */
export function createNotionCardSync(config) {
  const { dbId, tokenEnvs, logPrefix, workerPropName, optionalProps } = config;
  const getToken = () => tokenEnvs.map((k) => process.env[k]).find(Boolean) || null;
  ...
  async function getSchema() {
    ...
    let titleProp = null;
    let statusProp = null;
    let workerProp = null;   // 担当者 select。無いDBでも動く (担当者連携だけスキップ)
    ...
    for (const [name, def] of Object.entries(db.properties || {})) {
      ...
      if (def.type === 'select' && normName(name) === normName(workerPropName)) {
        workerProp = { name, type: def.type };
      }
      for (const [key, spec] of Object.entries(optionalProps || {})) {
  ...
  /** 担当者 select の選択肢一覧 (作業者マスタ取込用)。 */
  async function fetchWorkerNames() {
    const db = await notionFetch(`/databases/${dbId}`);
    if (!db) return [];
    const prop = db.properties?.[workerPropName];
   ...
   */
  async function syncStatus(folderName, statusLabel, jstDate, workerName = null, times = null) {
    if (!getToken()) return 'disabled';
    if (!folderName) return 'no_card';
    const card = await findTodayCard(folderName, jstDate);
    if (!card) {
      console.warn(`[${logPrefix}] ${jstDate} の「${folderName}」カードが見つかりません (${statusLabel} へ変更なし)`);
      return 'no_card';
    }
    const schema = await getSchema();
    ...
    console.log(`[${logPrefix}] ${folderName} → ${statusLabel}${workerName ? ` (担当: ${workerName})` : ''}`);
    return 'updated';
  }
   ...
   *
   * @param getBatchState () => ({folderName, workDate, label|null, workerName?, times?}) — 実行時点の状態を返す
   */
  const _batchQueues = new Map();   // batchId → Promise (直列化チェーン)
  function enqueueBatchSync(batchId, getBatchState) {
    if (!getToken()) return;
    ...
    const next = prev
      .then(async () => {
        ...
        try {
          await syncStatus(state.folderName, state.label, state.workDate, state.workerName, state.times);
        } catch (e) {
          // 一時障害向けに1回だけ再試行 (それでも失敗したらログのみ = fail-soft。
          // Notion同期は投影で、正本の計測データは picking.db / packing 側にある)
          await new Promise((r) => setTimeout(r, 5000));
          const retryState = getBatchState();
          if (!retryState || !retryState.label) return;
          await syncStatus(retryState.folderName, retryState.label, retryState.workDate, retryState.workerName, retryState.times);
        }
      })
      .catch((e) => console.warn(`[${logPrefix}] 同期失敗 (batch=${batchId}): ${e.message}`));
  ...
  return { getSchema, fetchWorkerNames, findTodayCard, syncStatus, enqueueBatchSync };
}

// ─── picking インスタンス (従来の exports を維持) ───

const _picking = createNotionCardSync({
  dbId: DAILY_DB_ID,
  tokenEnvs: ['PICKING_NOTION_TOKEN'],
  logPrefix: 'picking-notion',
  workerPropName: 'ピッキング担当者',
  optionalProps: {
...
export const getSchema = _picking.getSchema;
export const fetchNotionWorkerNames = _picking.fetchWorkerNames;
export const syncPickingStatus = _picking.syncStatus;
export const enqueueBatchSync = _picking.enqueueBatchSync;
