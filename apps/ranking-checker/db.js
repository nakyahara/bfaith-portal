  ...
  return {
    ...
    finishRun: db.prepare(`
      UPDATE run_state
       ...
       WHERE run_id = @run_id
    `),
    // 6時間以上前に開始された 'running' のみ stale とみなして failed にする。
    // 別プロセスが活きている run を誤って潰さないための年齢閾値。
    // 実測で 1 run に 3時間超かかることがある (2026-07-12: 3,566商品で3h12m) ため、
    // 旧閾値 3h では生きている run を failed 化し得た。実行時間 + 余裕で 6h とする。
    // 境界: Runner 側は `ageMs < 6h*ms` を fresh (skip) とするため、SQL は `>= 6h` で
    //       「ちょうど6時間」を stale 側に寄せる。
    markStaleRunning: db.prepare(`
      UPDATE run_state
         SET status = 'failed',
             error = COALESCE(error, '') || 'marked stale at next startRun (>=6h)',
             ended_at = strftime('%Y-%m-%d %H:%M:%S', 'now')
       WHERE status = 'running'
         AND (julianday('now') - julianday(started_at)) * 24 >= 6
    `),
