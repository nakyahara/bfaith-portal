...
生成6項目 + 裏取りで確認した事実の要約を Codex に渡して検品させる:

```bash
codex exec --skip-git-repo-check "楽天商品ページの文章の検品をお願いします。severity付き (critical/high/medium/low) で指摘してください。
検品観点: ①事実整合 (下記の裏取り事実に無い仕様・効果を書いていないか) ②景表法・薬機法・楽天規約の禁止表現 ③文字数 (楽天タイトル110字/Yahoo75字) ④誤字・不自然な日本語 ⑤訴求力の改善余地
[裏取り事実] ...
[生成物6項目] ..." </dev/null
```
