...
export const JOBS_REGISTRY = [
  ...
  },
  {
    ...
    purpose: 'ロジザード 入荷状況照会[FA04_01] から「対象データ=受付済 / 検索範囲=当日から7日前まで」の CSV を'
      ...
      + '⭐0件の日でも CSV は作って Drive まで送る (作る側はそれで正常)。'
      + '🚨ただし**取込側 (Render) は 0 件の CSV を取り込まない** — 0 行を「全部消えた」と読むと、'
      + '確認ずみの行き先といろはの在庫化カードが一斉に取り消されるため (2026-09-08 の事故)。'
      + 'いまの一覧をそのまま残すのが正しい振る舞い。取り込ませたい日は管理画面で中身を見て人が押す',
