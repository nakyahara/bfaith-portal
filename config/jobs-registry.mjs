...
export const JOBS_REGISTRY = [
  ...
  // ─────────────── scheduled_job (miniPC Task Scheduler) ───────────────
  {
    ...
    purpose: '全出品の想定利益 (単品販売シナリオ) を夜間計算。'
      ...
      + '+ au PAY searchItemInfos/searchStocks + Qoo10 GetAllGoodsInfo/GetItemDetailInfo/GetGoodsInventoryInfo '
      + '+ LINEギフト CMS items) → モール登録価格の取得 → '
    ...
    runbook: '🚨 成功 ping は「Render の公開ポインタが対象世代になった」ことを読み戻して確認してから打つ。'
      ...
      + 'AUPAY_PROXY_URL / AUPAY_PROXY_SECRET / AUPAY_SHOP_ID / QOO10_CERT_KEY / LINEGIFT_ACCESS_TOKEN / LINEGIFT_SHOP_ID / RENDER_MIRROR_URL / MIRROR_SYNC_KEY / JOBS_MONITOR_TOKEN。'
      + '【2026-09-19 追加】Yahoo (#1370) と au PAY (#1372) を足した。'
      + '【2026-09-20 追加】Qoo10 (#1373) も足した。'
      + '取得は Yahoo が約 6 分 (商品ごとに詳細を引く) / au PAY が約 40 秒 (一覧 API 2 本だけ) / Qoo10 が約 2 分。'
      + 'Qoo10 は 1 商品につき 3 本叩く (一覧 / 詳細 = 価格 / 在庫 = オプションの子コード)。'
      + '【2026-09-20 追加】LINEギフト (#1375) も足した (一覧 36 ページ 12 秒 + 詳細 3,594 件 約 2 分)。'
      + 'メルカリは出品を列挙する経路が無い (mercari-sync は楽天→CSV の一方通行・raw_mercari_orders も 0 行) ので入れていない。'
      + '🚨 オプションが取れなかった商品は行を作らない (「オプションなし」と決めつけると親の原価で取り違える)。'
  ...
  {
    id: 'logizard-nyuka-csv',
    type: 'scheduled_job',
    importance: 'P2',
    owner: '中原さん',
    purpose: 'ロジザード 入荷状況照会[FA04_01] から「対象データ=受付済 / 検索範囲=当日から7日前まで」の CSV を'
  ...
  // 交換当日に ping する前提で、その遅れと余裕のぶん Amazon の 180 日より 5 日短くしてある。
  {
    ...
    runbook: '① Solution Provider Portal にログイン → アカウント「雑貨イズム」→ アプリ一覧の「利益計算ツール」'
      ...
      + '🚨当日に打てなかったら、ping だけ後から打たない (監視の期限が Amazon の期限より後ろにずれる)。①から交換し直して、その当日に打つ '
  ...
  },
  {
    ...
    runbook: '① Google Cloud Console で Google Drive API を有効にし、OAuth クライアント (種類 = デスクトップ アプリ) を作る。'
      ...
      + '未審査のアプリの警告が出るが、自分たちで使う分には「詳細」→ 続行で認可できる。'
      ...
      + 'ブラウザのある PC で rclone authorize "drive" "<client_id>" "<client_secret>" を流し、'
      + '**ログイン画面のメールアドレスがその remote のアカウントと同じか見てから**許可し、出た結果を貼る '
      + '(ファイルが見えるかでは確かめられない = 別のアカウントにも同じ権限があれば通ってしまう) → '
      ...
      + '(drive.file の新しい client は旧 client のフォルダを見られない。見えたら scope が drive になっている = やり直す) '
      ...
      + '④ 差し替え = 1 回の時間帯で全部やる (新しい設定の rclone が旧フォルダの名前を変える前に 1 回でも動くと、bfaith-backup が 2 つになる)。'
      ...
      + ' (5) 止めたタスクを Enable-ScheduledTask で戻す。'
      + '途中で止めるとき: (3) の前なら rclone.conf.bak-<日付> を rclone.conf に戻してタスクを戻す。'
      + '(4) の後なら、フォルダの名前も bfaith-backup に戻す (旧 client の認可は生きているので元どおり動く)。'
      ...
      + 'Drive で新しい bfaith-backup の下に render・warehouse・logizard-history・mall-items-history ができ、bfaith-backup が 1 つしか無いこと。'
      + '初回は履歴 (logizard-history・mall-items-history) を全部送り直すので、その回だけ時間がかかる。'
      ...
      + '(撤去期限 = 切り替えの日 + 13 か月 = 新しいフォルダに月次の控えがそろうまで。それまでは旧フォルダから Drive の画面で取り出せる)。'
