...
--   🚨 ロックは 注文 → 明細 の順 (upsert_order と同じ = deadlock を避ける)。候補はロックを取ってから確定し、update のときにも「まだ未解決で元のコードが同じ」を再確認する
--      (受け口の push が同じ明細を変えて commit した後に、古い候補で上書きしない。Codex #1410 R1 #3)
...
declare
  ...
  v_orders integer;
begin
  -- ① 対象の注文を先にロック (注文 → 明細 の順)
  create temp table _rr_orders on commit drop as
    select o.order_id from core.orders o
     ...
     order by o.order_id;
  perform 1 from core.orders o join _rr_orders r on r.order_id = o.order_id order by o.order_id for update of o;
  -- ② 明細をロックしてから候補を確定 (ロックの後に読む = ここから先は push が明細を変えられない)
...
comment on function core.reresolve_order_lines(smallint, text, date) is '出品に当たらなかった注文明細を解き直す (出品が増えた後・注文日が p_since 以降)。当たった注文の updated_at を進めて売上日次の作り直しに乗せる。ロックは注文 → 明細';
