...
- **2 本目その 3 (9/23)**: W10 回復していない取込の異常 (`ops.ingest_runs` で `W10_SINCE` 以降 (started_at の JST の日) に始まった run のうち failed / partial / 120 分を超えて running のまま、**かつ回復していない**もの。前提なし。2026-10-07 までは info、その後 error)。🚨 **回復は種類ごとに決める**。後から閉じた run・行の世代が進んだ だけでは回復にしない (Codex D2 / #1417 R1):
  - 注文 5 モール・NE の出荷 (chunk で送る取込): **悪い run の後に始まった今朝の差分送信 (incremental) が W7 と同じ判定で pass** (同じ実行 ID・失敗 / stale / 整形できない が 0・送ったなら Render の run が success) のときだけ回復。差分送信は ack されなかった行 (失敗した行・止まった run で送れなかった行 = outbox から追跡対象に引き継がれる) を必ず送り直す。範囲送信 (`--from/--to`) は追跡対象を送り直さない = 証明にならない。さらに受け取った chunk の失敗した行 (`ops.ingest_chunks.result.failed` = 全件。run の `failed_ranges` は 200 件で切れる) は、今の行 (`core.orders` / `core.shipments`) の `received_batch_seq` が run の世代 (`checksum` = batch_seq) より大きいことも見る (行が無い・同じ世代は残る)。失敗の行数と読めた鍵の数が合わない run も残る。出荷の証跡は `shipments` (W7 は見ない)
