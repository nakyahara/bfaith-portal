...
- **2 本目その 3 (9/23)**: W10 回復していない取込の異常 (`ops.ingest_runs` で `W10_SINCE` 以降に始まった run のうち failed / partial / 120 分を超えて running のまま、**かつ回復していない**もの。前提なし。2026-10-07 までは info、その後 error)。🚨 **回復は種類ごとに決める** (後から来た無関係な成功で回復にしない。Codex D2):
  - 注文 5 モール・NE の出荷 (chunk で送る取込): partial = 失敗した行 (`ops.ingest_chunks.result.failed` = 全件。run の `failed_ranges` は 200 件で切れる) の今の行 (`core.orders` / `core.shipments`) の `received_batch_seq` が、その run の世代 (`checksum` = batch_seq) より大きい = 取り直された。行が無い・同じ世代は残る。失敗の行数があるのに鍵が読めない run も残る (回復を確かめられない)。止まった running (送り手が途中で落ちた・daily-sync の 30 分で切られた = 誰も片づけない) = 送れなかった行は miniPC の outbox から次の run に引き継がれるので、同じ種類で世代が新しい run が閉じていれば (complete) 回復。受け取った chunk の失敗した行は同じく行ごとに見る
  - ロジザードの毎時の在庫: 同じか新しい世代 (`checksum` = 取得時刻) の run が success・complete なら回復 (状態の写し = 新しい世代が入れば古い世代の失敗は残らない。失敗そのものは jobs-monitor の ping がその時に知らせる)
  - 見ない種類 (`W10_DELEGATED`): 在庫の日次 (W1 / W2 が `stock_capture_days` で見る。1 取引なので running / failed の行は残らない) / 夜間ロード (成功したときだけ行を作る)。どちらにも無い種類の悪い run は `other/*` で異常 = `W10_KINDS` か `W10_DELEGATED` に足す
  - 今朝の push の run (証跡 `orders-<mall>` の run_id) は W7 が見る = W10 は数えない (同じ失敗を 2 回出さない。翌朝も残っていれば W10)。案件は取込の種類ごとに 1 つ (同じ行が毎日失敗し続けても「新・回復」をくり返さず「継続 N 日」)・未回復の run は明細 (失敗した行の数・取り直されていない行の例 3 件)
  - 直せないと決めた run は `W10_ACCEPTED_RUNS` に理由と責任を書いて外す (黙って消さない)
