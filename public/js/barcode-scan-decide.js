...
(function (root, factory) {
   ...
   *   codes = 解析できた / error = 解析が例外で落ちた / idle = 映像がまだ来ていない
   * @returns {{state: object, action: 'continue'|'accept'|'abort', value?: string}}
   */
  function step(state, event) {
    ...
    if (kind === 'error') {
      ...
      var next = { lastSeen: null, repeats: 0, failStreak: fs };
      return { state: next, action: fs >= MAX_FAIL_STREAK ? 'abort' : 'continue' };
    }
    if (kind !== 'codes') {
