...
await ta('[!] 期待する表がすべてある', async () => {
  ...
  const expect = [
    ...
    'core.finance_source_policy', 'core.order_finance_daily',
  ];
  ...
  assert.deepEqual(views.map((v) => v.t).sort(), ['mart.v_cross_mall_diff', 'mart.v_listing_360', 'mart.v_order_finance_summary', 'mart.v_product_360', 'mart.v_product_dq', 'mart.v_sku_stock', 'mart.v_warehouse_stock_current']);
});
