...
await ta('[!] 期待する表がすべてある', async () => {
  ...
  assert.deepEqual(views.map((v) => v.t).sort(), ['mart.v_cross_mall_diff', 'mart.v_finance_daily_legacy', 'mart.v_listing_360', 'mart.v_order_finance_summary', 'mart.v_order_finance_uncovered', 'mart.v_product_360', 'mart.v_product_dq', 'mart.v_purchase_backorder_by_sku', 'mart.v_purchase_order_open', 'mart.v_sales_daily', 'mart.v_sales_daily_check', 'mart.v_shipments_daily', 'mart.v_shipments_unlinked', 'mart.v_sku_stock', 'mart.v_warehouse_stock_current']);
});
