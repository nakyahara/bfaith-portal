...
import { ingestStockDay, validateStockDayBody, stockChecksum, STOCK_SOURCES } from '../apps/company-db/ingest/stock-daily.mjs';
...
const body = (date, rows, x = {}) => ({ source: 'ne', scope: 'main', snapshot_date: date, captured_at: `${date}T22:01:23.005Z`.replace(date, date), rows, ...x });
...
await t('途中で落ちたら全部巻き戻る: building の日も、行も、取込の記録も残らない (= 次の送信がそのまま通る)', async () => {
  ...
  assert.deepEqual([await dayOf('2026-03-19'), (await rowsOf('2026-03-19')).length, Number((await one(`select count(*)::int as n from ops.ingest_runs where entity = 'stock_daily' and status = 'running'`)).n)], [undefined, 0, 0]);
...
const put = (date, code, qty) => wh.prepare(`insert or replace into ne_stock_daily_snapshot (business_date, 商品コード, 在庫数, captured_at) values (?, ?, ?, ?)`).run(date, code, qty, `${date}T22:01:23.005Z`);
