...
import { buildMasterSyncParts, partSize, assertPartFits, MAX_PART_BYTES, RECEIVER_LIMIT_BYTES } from '../apps/warehouse/sync-to-render.js';
...
t('出荷サマリはマスタとは別の部 (受け口は鍵ごとに全件置換)。state ok なら 0 件でも送る', () => {
...
t('stale / failed のときは出荷サマリを送らない (Render は前回分を保持)。マスタに混ぜたら例外', () => {
...
t('大きさはバイト数で見る (日本語は 1 文字 3 バイト = 文字数では小さく見える)', () => {
...
t('受け口の上限 (12MB) に余裕を見た 11MB を超える部は送る前に止める。以内なら通る (2026-09-15 の実測値で)', () => {
