...
## au PAY ストアクーポン 月次入れ替え (2026-07-31 実装・毎日09:45自動実行)

「商品N点以上で X円引き」クーポン **4本 (2/3/4/5点)** を、毎月25日に翌月分へ自動で作り直す。
中原さんが毎月末に手作業でやっていたもの。Yahoo版 (`yahoo-coupon-rotate.mjs`) と同じ考え方。

```powershell
node scripts/mall-csv-fetcher/aupay-coupon-rotate.mjs            # dry-run (確認画面まで、作成しない)
node scripts/mall-csv-fetcher/aupay-coupon-rotate.mjs --live     # 実作成
node scripts/mall-csv-fetcher/test-aupay-coupon-lib.mjs          # 純ロジックのテスト (オフライン)
node scripts/mall-csv-fetcher/aupay-coupon-survey.mjs            # 画面調査 (読み取り専用ワンオフ)
```

miniPC タスクスケジューラ `AupayCouponRotate` (毎日09:45 / bfaith・Interactive・最高権限、
`C:\tools\mall-csv-fetcher\run-aupay-coupon.ps1`)。Yahoo版 09:30 と15分ずらしてある。
**毎日動くが作るのは25日以降で未作成のときだけ**、それ以外の日は通知も出さない。

### 押さえておくべき前提 (実測 2026-07-31)

- au PAY マーケットに**クーポン発行APIは無い** → Wow!manager の画面操作が唯一の手段
- **登録後のクーポンは編集できない** (詳細画面から押せるのは「配布停止」だけ)
- 一覧の「コピー」は素のURL `/distributionCoupon/copyCreate/<couponId>` で開ける。
  コピーすると**日付欄だけが空になる**ので、期間を入れ直して登録するのが正規の月次運用
- ⭐**配布期間の終了は開始の「2時間後〜30日後」まで**。31日ある月に1日開始・月末終了はできない
  → 31日月は2日開始 (8/02 00:00〜8/31 23:59)、30日以下の月は1日開始。
  現行の手作業クーポンが「7/02〜7/31」だったのは日付のズレではなくこの制約によるもの
- 日付欄は readonly + jQuery UI datepicker (**書式 `yy/mm/dd`**)。fillが効かないので
  ネイティブsetterで入れる。分秒は画面が自動で 00 / 59 を入れる
- Yahoo版と違い**クーポン画像を使っていない** ("画像登録なし") ので、画像と金額のズレは起きない
- 確認画面の登録ボタンは `input[name=btnSave_update]` "クーポンを登録する"
  (同じ画面に "作成内容を修正する" もあるので name で厳密に指定する)

### 期間と金額のルール (中原さん確定 2026-07-31)

| 項目 | 値 |
|---|---|
| 配布期間 | (翌月1日 または 月末-29日の遅い方) 00:00 〜 翌月末日 23:59 |
| 利用期間 | 配布と同じ開始 〜 配布終了の翌日 23:59 |
| 金額 | 2点 30⇔33 / 3点 50⇔55 / 4点 100⇔110 / 5点 150⇔155 を毎月交互 |
| 作成日 | 毎月25日 (`ACOUPON_CREATE_DAY`)。26日以降でも未作成なら作る = 失敗した日の挽回 |

金額を振るのは同額を続けると恒常値引きになるため。「今の金額の次」を巡回リストから選ぶ方式なので
状態ファイルは持たない (冪等)。**現在の金額が巡回リストに無ければ何もしない** (安全側)。

### ガード

- 既定は dry-run。`--live` のときだけ登録する
- 遷移先を `/wmshopclient/(shopCouponMgt|distributionCoupon)/` 配下に限定し、
  停止・削除系のパス (`detailStop` 等) に入ろうとしたら即中止
- 二重作成防止: **その月の利用開始クーポンが既にあればスキップ** (1日開始/2日開始どちらでも検出)
- **一覧が判定に使えるかを先に検証** (`checkListCoverage`)。一覧はページングされるので、
  対象月の既存クーポンが次ページに押し出されると「未作成」と誤判定して二重作成しうる。
  そこで `psz=200` で取得したうえで ①利用開始日の降順に並んでいる ②最古の行が対象月より前まで
  届いている ③利用期間を読めない行が1件も無い の3つを確認し、1つでも欠ければ登録前に中止する
- **対象月に名前を解析できないクーポンがあれば中止** (表記違いの同種クーポンを二重に作らないため。
  別種のクーポンを手で作った月は `ACOUPON_IGNORE_UNPARSED=1` で続行)
- **多重起動ロック**: 定時実行の最中に手で `--live` を叩くと同じクーポンを2本作りうるため、
  `logs/aupay-coupon.lock` を `open(…,'wx')` で取る。**取得経路はこれ1本だけで、自動回収はしない** —
  「古いロックを確認してから消す」経路を作ると、確認と削除の間に別の実行が取り直したロックを
  消してしまい二重登録につながるため (Codexレビュー3〜5巡目)。異常終了でロックが残ったら
  エラー通知の指示どおり手で消す (毎日走るので1日止まっても月末まで取り返せる)
- 登録前に2段階で検証 — ①フォームの読み戻し ②確認画面の全項目照合
  (名前・説明・割引額・配布期間・利用期間・まとめ買い点数・配布種別・広告表示制御ほか)。
  1つでも食い違えば登録しない
- コピー元は「クーポン名の金額」と「割引種別の実額」が一致する行だけ (名前と実額のズレを伝播させない)
- 登録後に一覧で実在確認 (点数・金額・利用開始日で照合。
  ⚠️一覧のクーポン名は配布中だと「獲得用URLコピー」が続くので名前の完全一致では見つからない)
