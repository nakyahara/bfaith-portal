...
const EXPECTED_IDS = [
  ...
  'master-decisions',       // 2026-09-26 追加 (マスタの判断 = 照合 ② の NE との差・D2')
];
const SPECIAL_PATHS = {
  ...
  'master-decisions': '/apps/master-decisions/',
};
const ids = apps.map(a => a.id);
ok(ids.length === 56 && new Set(ids).size === 56, `アプリは 56 本で id の重複なし (実際 ${ids.length} 本)`);
...
ok(admin.totalCards === 45, `管理者: トップのカードは 45 枚 (実際 ${admin.totalCards})`);
...
ok(EXPECTED_IDS.every(id => adminIds.filter(x => x === id).length === 1), '管理者: 56 本すべてが 1 回ずつ (カードかグループのボタンで) 出る');
...
ok(EXPECTED_IDS.every(id => hrefs(adminHtml).includes(SPECIAL_PATHS[id] || `/apps/${id}`)), '描画 (管理者): 56 本すべてのリンクがある');
