...
console.log('\n=== Test 14: classifyProduct 分類網羅 ===');
{
  ...
  //   設計書§14: new_product_flag (手動) と launch_date < 12ヶ月 のいずれかで新商品扱い。
  //   muskblend 系のような新商品が手動 flag 未設定でも 撤退候補 に流入しないことを保証。
  const { isNewProductByLaunchDate, NEW_PRODUCT_WINDOW_DAYS } =
    await import('../apps/profit-analysis/candidates.js');

  expectEq(NEW_PRODUCT_WINDOW_DAYS, 365, '14q NEW_PRODUCT_WINDOW_DAYS=365');

  // 純関数: 形式バリエーション
  expectEq(isNewProductByLaunchDate(null, '2026-04-21'), false, '14q null → false');
  expectEq(isNewProductByLaunchDate('', '2026-04-21'), false, '14q 空文字 → false');
  expectEq(isNewProductByLaunchDate('invalid', '2026-04-21'), false, '14q 不正 → false');
  expectEq(isNewProductByLaunchDate('2026-04-01', '2026-04-21'), true, '14q YYYY-MM-DD 直近 → true');
  expectEq(isNewProductByLaunchDate('2026/04/01', '2026-04-21'), true, '14q YYYY/MM/DD 直近 → true');
  expectEq(isNewProductByLaunchDate('2026/04/01 10:30:00', '2026-04-21'), true,
    '14q YYYY/MM/DD HH:MM:SS 直近 → true');
  // 未来日付: 誤入力扱い → false
  expectEq(isNewProductByLaunchDate('2027-01-01', '2026-04-21'), false, '14q 未来日付 → false');
  // 境界: ちょうど 364日前 → true、365日前 → false (strict <)
  expectEq(isNewProductByLaunchDate('2025-04-22', '2026-04-21'), true, '14q 364日前 → true');
  expectEq(isNewProductByLaunchDate('2025-04-21', '2026-04-21'), false, '14q 365日前 → false (strict <)');
  // 1年以上前 → false
  expectEq(isNewProductByLaunchDate('2024-04-01', '2026-04-21'), false, '14q 1年以上前 → false');
  // 不正日付は false（Codex R1 Medium 反映: Date.UTC 繰り上げで誤判定しない）
  expectEq(isNewProductByLaunchDate('2026-13-01', '2026-04-21'), false, '14q 13月 → false');
  expectEq(isNewProductByLaunchDate('2026-02-30', '2026-04-21'), false, '14q 2月30日 → false');
  expectEq(isNewProductByLaunchDate('2025-02-29', '2026-04-21'), false, '14q 非うるう年 2/29 → false');
  // タイムゾーン非依存: today を Date オブジェクトで渡しても launch_date 文字列と日単位で比較される
  //   Codex R1 High 反映: JST 真夜中時間帯でも当日発売品が「未来」扱いされない
  expectEq(isNewProductByLaunchDate('2026-04-21', new Date('2026-04-21T00:30:00+09:00')), true,
    '14q JST 真夜中: 同日発売は新商品（未来扱いされない）');
  expectEq(isNewProductByLaunchDate('2026-04-21', new Date('2026-04-21T23:30:00+09:00')), true,
    '14q JST 夜遅く: 同日発売は新商品');

  // classifyProduct 統合: launch_date 直近 + manual_flag=0 → 新商品保留
  //   muskblend100ml シナリオ: 売上0、在庫あり、launch_date 直近
  r = classifyProduct(mkRow({
    new_product_flag: 0,
    new_product_launch_date: '2026-03-01',  // 51日前 → 新商品
    daily_last_sale: null,
    monthly_last_month: null,
    sales_period: 0, sales_30d: 0, sales_90d: 0,
    latest_stock: 50, avg_stock: 50,
  }), thresholds, opts);
  expectEq(r.classification, '分類外', '14q 新商品(launch_date直近) → 分類外');
  expectEq(r.reason, '新商品保留', '14q 新商品(launch_date直近) 理由');

  // 手動 flag=0 + launch_date 1年超 → 既存ロジック通り撤退候補に流入
  r = classifyProduct(mkRow({
    new_product_flag: 0,
    new_product_launch_date: '2024-01-01',  // 1年以上前
    daily_last_sale: null,
    monthly_last_month: '2024-12',          // 365日超販売なし
    sales_period: 0, sales_30d: 0, sales_90d: 0,
  }), thresholds, opts);
  expectEq(r.classification, '撤退候補', '14q launch_date 1年超 + 365日販売なし → 撤退候補');

  // 手動 flag=1 が最優先（launch_date が古くても新商品扱い）
  //   再リリース等のケース: 古い商品コードを新商品として扱い直す運用
  r = classifyProduct(mkRow({
    new_product_flag: 1,
    new_product_launch_date: '2020-01-01',
    daily_last_sale: null,
    monthly_last_month: null,
    sales_period: 0, sales_30d: 0, sales_90d: 0,
  }), thresholds, opts);
  expectEq(r.classification, '分類外', '14q manual_flag=1 は launch_date を上書き');
  expectEq(r.reason, '新商品保留', '14q manual_flag=1 の理由');

  // launch_date が NE 形式 "YYYY/MM/DD HH:MM:SS"（NE API レスポンス想定）
  r = classifyProduct(mkRow({
    new_product_flag: 0,
    new_product_launch_date: '2026/03/15 09:00:00',
    daily_last_sale: null,
    monthly_last_month: null,
    sales_period: 0, sales_30d: 0, sales_90d: 0,
  }), thresholds, opts);
...
// ───────────────────────────────────────────────
// Test 5b: launch_date 解決ヘルパー（rebuild-m-products → candidates.js 連携）
//   NE 作成日（goods_creation_date）の自動コピーが優先順位通りに動くことを保証する。
//   既存 m_products 値（手動 or 過去の自動引き継ぎ）がある場合は NE 値で上書きしないこと、
//   無い場合は NE 値（さまざまな形式）を 'YYYY-MM-DD' に正規化することを検証。
// ───────────────────────────────────────────────
console.log('\n=== Test 5b: launch_date 解決ヘルパー（NE 作成日 自動コピー） ===');
{
  const { normalizeNeCreationDate, resolveLaunchDate } =
    await import('../apps/warehouse/rebuild-m-products.js');

  // normalizeNeCreationDate: 形式バリエーション
  expectEq(normalizeNeCreationDate(null), null, '5b normalize null');
  expectEq(normalizeNeCreationDate(''), null, '5b normalize 空文字');
  expectEq(normalizeNeCreationDate('invalid'), null, '5b normalize 不正');
  expectEq(normalizeNeCreationDate('2026-04-01'), '2026-04-01', '5b normalize YYYY-MM-DD');
  expectEq(normalizeNeCreationDate('2026/04/01'), '2026-04-01', '5b normalize YYYY/MM/DD');
  expectEq(normalizeNeCreationDate('2026/4/1'), '2026-04-01', '5b normalize ゼロパディングなし');
  expectEq(normalizeNeCreationDate('2026/04/01 09:30:00'), '2026-04-01',
    '5b normalize NE API 形式 (YYYY/MM/DD HH:MM:SS)');
  expectEq(normalizeNeCreationDate('2026-04-01T09:30:00'), '2026-04-01', '5b normalize ISO');
  // 不正日付は null に倒す（Date.UTC 繰り上げによるデータ汚染防止 / Codex R1 Medium 反映）
  expectEq(normalizeNeCreationDate('2026-13-01'), null, '5b normalize 13月 → null');
  expectEq(normalizeNeCreationDate('2026-02-30'), null, '5b normalize 2月30日 → null');
  expectEq(normalizeNeCreationDate('2026-04-31'), null, '5b normalize 4月31日 → null');
  expectEq(normalizeNeCreationDate('2026/00/15'), null, '5b normalize 0月 → null');
  expectEq(normalizeNeCreationDate('2026/04/00'), null, '5b normalize 0日 → null');
  // うるう年: 2024-02-29 は valid, 2025-02-29 は invalid
  expectEq(normalizeNeCreationDate('2024-02-29'), '2024-02-29', '5b normalize うるう年 valid');
  expectEq(normalizeNeCreationDate('2025-02-29'), null, '5b normalize 非うるう年 2/29 → null');

  // resolveLaunchDate: 優先順位（手動 > NE 作成日 > null）
  expectEq(resolveLaunchDate(null, null), null, '5b resolve 両方 null');
  expectEq(resolveLaunchDate(null, '2026-03-01'), '2026-03-01', '5b resolve 手動なし → NE 採用');
  expectEq(resolveLaunchDate(null, '2026/03/01 10:00:00'), '2026-03-01',
    '5b resolve 手動なし → NE 採用（NE形式正規化）');
  expectEq(resolveLaunchDate('2025-12-15', null), '2025-12-15', '5b resolve 手動あり → 手動優先');
  expectEq(resolveLaunchDate('2025-12-15', '2026-03-01'), '2025-12-15',
    '5b resolve 手動あり → NE があっても手動優先');
  // NE 形式が壊れているケース
  expectEq(resolveLaunchDate(null, 'broken'), null, '5b resolve NE 不正 → null');
  // carryover が壊れている場合は NE にフォールバック（Codex R2 Medium 反映）
  expectEq(resolveLaunchDate('broken', '2026-03-01'), '2026-03-01',
    '5b resolve carryover 不正 → NE 採用');
  expectEq(resolveLaunchDate('2026-13-01', '2026-03-01'), '2026-03-01',
    '5b resolve carryover 13月 → NE 採用');
  expectEq(resolveLaunchDate('broken', null), null,
    '5b resolve carryover 不正 + NE なし → null');
  // valid な carryover は形式正規化される（"YYYY/MM/DD" → "YYYY-MM-DD"）
  expectEq(resolveLaunchDate('2025/12/15', '2026-03-01'), '2025-12-15',
    '5b resolve carryover valid だが形式違い → 正規化して採用');

  console.log('[OK] launch_date 解決ヘルパー動作確認完了（5b）');
}
