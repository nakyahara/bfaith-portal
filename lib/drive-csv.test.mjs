    ...
    pagesByFolder: {
      F1: [
        [file('hit', '送り状データダウンロード_202609191500.csv', '2026-09-19T06:00:00Z'),
         file('zzz', '関係ないファイル.csv', '2026-09-18T00:00:00Z')],
        [file('older', '送り状データダウンロード_202609181137.csv', '2026-09-18T02:37:00Z')],
      ],
    },
  });
  const info = await findDriveFile(CLOUD, { drive: m.drive });
...
test('latestPrefix: 同時刻の候補が尽きずページ上限に達したら「確定できない」で止める', async () => {
  ...
  await assert.rejects(() => findDriveFile(CLOUD, { drive: m.drive }), /確定できません/);
});
