...
function mockDrive({ filesByFolder = {}, incomplete = null, failOn = null, delayMs = 0 } = {}) {
  const calls = [];
  let inflight = 0;
  let maxInflight = 0;
  const drive = {
    files: {
      ...
      list: async (params) => {
        ...
        calls.push({ q: String(params.q), folderId });
        inflight++;
        ...
        try {
          ...
          return {
            data: {
              ...
              incompleteSearch: incomplete === folderId,
            },
          };
        } finally {
          inflight--;
...
test('mapWithConcurrency: 不正な同時実行数でも止まらない (0/負数/NaN は1扱い)', async () => {
