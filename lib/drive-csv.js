...
async function findLatestCandidate(drive, cfg, driveId, esc) {
  ...
  for (let i = 0; i < LATEST_MAX_PAGES; i++) {
    ...
    const hits = (list.data.files || []).filter((f) => isCandidate(f.name));
    if (hits.length) {
      // ページ内の並びは API 任せにせず自前で決める。同時刻ならファイル名 (末尾が日時) の大きい方
      hits.sort((a, b) => String(b.modifiedTime || '').localeCompare(String(a.modifiedTime || ''))
        || nfc(b.name).localeCompare(nfc(a.name)));
      return hits[0];
    }
    pageToken = list.data.nextPageToken;
    if (!pageToken) return null;
  }
  throw new Error(`Drive フォルダのファイルが多すぎて最新の対象ファイルを特定できません (${LATEST_PAGE_SIZE * LATEST_MAX_PAGES}件を超過)。古いファイルを整理してください。`);
}
