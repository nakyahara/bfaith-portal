...
async function findLatestCandidate(drive, cfg, driveId, esc) {
  ...
  for (let i = 0; i < LATEST_MAX_PAGES; i++) {
    ...
    for (const f of files) if (isCandidate(f.name) && isBetter(f, best)) best = f;
    ...
    if (best && files.length && files.some((f) => mtime(f) < mtime(best))) return best;
  }
  throw new Error(
    best
      ? `Drive フォルダに同じ更新日時のファイルが多すぎて、最新の1本を確定できません (${LATEST_PAGE_SIZE * LATEST_MAX_PAGES}件を超過)。古いファイルを整理してください。`
      : `Drive フォルダのファイルが多すぎて最新の対象ファイルを特定できません (${LATEST_PAGE_SIZE * LATEST_MAX_PAGES}件を超過)。古いファイルを整理してください。`
  );
}
