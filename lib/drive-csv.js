 ...
 * 切り替えて再試行する。allDrives は incompleteSearch がありうるため、立ったら警告を出す
 * (フォルダ指定の小規模検索では実害はまず無い)。
 */
async function listAllPages(drive, params, maxPages = 20) {
  ...
  for (let i = 0; i < maxPages; i++) {
    ...
    if (list.data.incompleteSearch) {
      console.warn('[drive-csv] incompleteSearch=true (allDrives検索で一部が漏れた可能性)');
    }
 ...
 * Drive API は再帰検索ができないため、'id' in parents を OR で束ねてクエリする (10件ずつ)。
 * 返り値の parent_name はどのサブフォルダにあったか (出荷_XX) の表示・導出用。
 */
export async function listDriveFilesAcross({ folders, nameContains, pageSize = 100 }) {
  if (!Array.isArray(folders) || folders.length === 0) return [];
  const { drive } = getDriveClient();
  const esc = (s) => String(s).replace(/\\/g, '\\\\').replace(/'/g, "\\'");
  const nameById = new Map(folders.map((f) => [f.folder_id, f.name]));
  let driveId = null;
  try {
    const meta = await drive.files.get(
      { fileId: folders[0].folder_id, fields: 'id, driveId', supportsAllDrives: true },
      { timeout: TIMEOUT }
    );
    driveId = meta.data.driveId || null;
  ...
  for (let i = 0; i < folders.length; i += 10) {
    const chunk = folders.slice(i, i + 10);
    const parentsQ = '(' + chunk.map((f) => `'${esc(f.folder_id)}' in parents`).join(' or ') + ')';
    const q = [parentsQ, 'trashed = false']
      .concat(nameContains ? [`name contains '${esc(nameContains)}'`] : [])
      .join(' and ');
    const files = await listAllPages(drive, {
      q,
      fields: 'nextPageToken, incompleteSearch, files(id, name, parents, modifiedTime, size)',
      orderBy: 'modifiedTime desc',
      pageSize,
      supportsAllDrives: true,
      includeItemsFromAllDrives: true,
    ...
    for (const f of files) {
      const parentId = (f.parents || []).find((p) => nameById.has(p)) || null;
      out.push({
        file_id: f.id,
        filename: f.name,
        parent_id: parentId,
        parent_name: parentId ? nameById.get(parentId) : null,
        size: f.size != null ? Number(f.size) : null,
        modified_time: f.modifiedTime || null,
        modified_time_jst: toJstDisplay(f.modifiedTime),
      });
