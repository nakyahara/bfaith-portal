...
// 「最新の1本」探しで辿るページ数の上限。更新日時の新しい順に辿り、best より古いファイルが
// 出た時点 (= 同時刻の候補が尽きた時点) で止めるので、通常は1ページ目で終わる。
// ここに達するのは「新しい側に対象外ファイルが積み上がっている」か「同じ更新日時のファイルが
// 延々と並んでいる」場合で、どちらも確定できないものとしてエラーにする (findLatestCandidate 末尾)。
const LATEST_PAGE_SIZE = 100;
const LATEST_MAX_PAGES = 10;

// ファイル名の比較は NFC に揃える (Mac から上げたファイルは濁点が分解された NFD で届くことがある)
const nfc = (s) => String(s ?? '').normalize('NFC');

/**
 * cfg.latestPrefix つきの設定用: 「固定ファイル名」または「接頭辞で始まる .csv」のうち、
 * 更新日時が最新の1本を返す (無ければ null)。
 *
 * ダウンロードのたびにファイル名が変わる出力 (例: ゆうプリクラウドの
 * 「送り状データダウンロード_YYYYMMDDhhmm.csv」) を、固定名へ上書きし直さずに読むためのもの。
 * 名前の絞り込みは Drive の `name contains` (トークン単位の一致で日本語の挙動が読みにくい) に頼らず、
 * 単一フォルダの列挙 (インデックスを介さない) をこちらで絞る。
 */
async function findLatestCandidate(drive, cfg, driveId, esc) {
  const fixed = nfc(cfg.filename);
  const prefix = nfc(cfg.latestPrefix);
  const isCandidate = (name) => {
    const n = nfc(name);
    return n === fixed || (n.startsWith(prefix) && /\.csv$/i.test(n));
  };
  const mtime = (f) => String(f?.modifiedTime || '');
  // 更新日時 → ファイル名 (末尾が日時) → id の順で比べる。**API の返却順に依存させない**。
  // id まで比べるのは、同時刻・同名の別ファイルがあっても毎回同じ1本を選ぶため
  // (揺れると downloadDriveCsv の DL 前後の再確認が空振りし、更新されていないのに
  //  「更新中です」で失敗し続ける。Codexレビュー 1巡目 P2)。
  const isBetter = (a, b) => {
    if (!b) return true;
    const t = mtime(a).localeCompare(mtime(b));
    if (t !== 0) return t > 0;
    const n = nfc(a.name).localeCompare(nfc(b.name));
    if (n !== 0) return n > 0;
  ...
  let best = null;
  let pageToken;
  for (let i = 0; i < LATEST_MAX_PAGES; i++) {
    const list = await drive.files.list({
      q: `'${esc(cfg.folderId)}' in parents and mimeType != 'application/vnd.google-apps.folder' and trashed = false`,
    ...
    // 欠けた一覧から「最新」を決めない (古いファイルを最新として取り込んでしまう)
    if (list.data.incompleteSearch) {
      throw new Error('Drive の検索結果が不完全です (incompleteSearch=true)。最新ファイルを取り違えないよう中断しました。少し待ってからもう一度お試しください。');
    }
    const files = list.data.files || [];
    for (const f of files) {
      if (!isCandidate(f.name)) continue;
      // 候補の更新日時が無ければ「どれが最新か」を決めようがない。欠けた値を空文字として
      // 比べると、日時のある候補を無条件に勝たせて取り違える (Codexレビュー 2巡目 P2)
      if (!mtime(f)) {
        throw new Error(`Drive が返した「${f.name}」に更新日時がありません。どれが最新かを決められないため中断しました。少し待ってからもう一度お試しください。`);
      }
      if (isBetter(f, best)) best = f;
    }

    pageToken = list.data.nextPageToken;
    if (!pageToken) return best;
    // 🚨 候補が見つかったページで即座に打ち切らない。orderBy は modifiedTime しか指定して
    // いないので、**同じ更新日時の候補がページ境界をまたいで続きうる** (次ページに名前の
    // 新しい回があっても見ずに返してしまう。Codexレビュー 1巡目 P2)。
    // 打ち切ってよいのは「このページに best より古いものが出てきた」= 以降のページは
    // 全部それより古い、と分かったときだけ。ページ内の並びには依存せず最小値で判定する。
    // 更新日時が無いファイルは「古い」証拠にならないので打ち切り根拠から外す (2巡目 P2)。
    if (best && files.some((f) => mtime(f) && mtime(f) < mtime(best))) return best;
  }
  // 上限はページ数 (1ページの実件数は pageSize 以下になりうる) なので、件数では書かない。
  // 「古いファイルを整理」では直らない (同時刻のファイル群が先頭ページを埋めている) ため、
  // 原因に沿った復旧手順を書く (Codexレビュー 2巡目 P3)。
  throw new Error(
    best
      ? `Drive フォルダに同じ更新日時のファイルが並んでいて、${LATEST_MAX_PAGES} ページ探しても最新の1本を確定できません。対象フォルダに残っている不要なCSVを別フォルダへ移すか、取り込みたいCSVを手動アップロードしてください。`
      : `Drive フォルダのファイルが多く、${LATEST_MAX_PAGES} ページ探しても対象ファイルが見つかりません。対象フォルダに不要なファイルが溜まっていないか確認するか、取り込みたいCSVを手動アップロードしてください。`
  );
}
 ...
 * cfg.latestPrefix があれば「固定名 or 接頭辞で始まる .csv」の最新1本を採る (findLatestCandidate)。
 * deps.drive はテスト用の差し込み口 (listDriveFilesAcross と同じ)。
 */
export async function findDriveFile(cfg, deps = {}) {
  assertConfig(cfg);
  const { drive, serviceAccountEmail } = deps.drive ? { drive: deps.drive, serviceAccountEmail: null } : getDriveClient();
  ...
  let file;
  if (cfg.latestPrefix) {
    file = await findLatestCandidate(drive, cfg, driveId, esc);
  } else {
    const list = await drive.files.list({
    ...
    file = list.data.files && list.data.files[0];
  }
  if (!file) {
    const wanted = cfg.latestPrefix ? `「${cfg.filename}」または「${cfg.latestPrefix}….csv」` : `「${cfg.filename}」`;
    throw vErr(`Drive フォルダに${wanted}が見つかりません (${label})。${cfg.notFoundHint || ''}`.trim());
  }
  return {
    label,
    filename: file.name,
...
// latestPrefix の有無で探し方 (=当たるファイル) が変わるのでキーにも含める
const cacheKey = (cfg) => `${cfg.folderId}/${cfg.filename}${cfg.latestPrefix ? `|latest:${cfg.latestPrefix}` : ''}`;
