...
export async function downloadDriveFileById({ fileId, folderId, maxBytes = DEFAULT_MAX_CSV_BYTES }) {
  ...
  const f = meta.data;
  if (f.trashed || !Array.isArray(f.parents) || !f.parents.includes(folderId)) {
    throw vErr('指定されたファイルは対象フォルダにありません。一覧を更新してやり直してください。');
  }
  if (f.size != null && Number(f.size) > maxBytes) {
    throw vErr(`ファイルが大きすぎます (${Math.round(Number(f.size) / 1024 / 1024)}MB > 上限${Math.round(maxBytes / 1024 / 1024)}MB)。`);
  }
